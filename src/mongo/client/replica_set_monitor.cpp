...
protected:
    void run() {
        ...
        while (!inShutdown() && !StaticObserver::_destroyingStatics) {
            ...
            //NOTE(zhenyipeng) Modify this timeout to 5 secs
            _stopRequestedCV.wait_for(sl, stdx::chrono::seconds(5));
        }
    }
    ...
    void checkAllSets() {
        ...
        for (const string& setName : globalRSMonitorManager.getAllSetNames()) {
			...
			LOG(0) << "schedule a thread to check replica set:" << setName;
            std::thread checkSetThread(&ReplicaSetMonitorWatcher::checkSet, this, setName);
            threadPool.push_back(move(checkSetThread));
        }

        LOG(0) << "schedule completed, wait for completion";

        for (auto &iter : threadPool) {
            iter.join();
        }

        LOG(0) << "checkAllSets completed";
    }

	void checkSet(const string setName) {
        LOG(0) << "checking replica set: " << setName;

        shared_ptr<ReplicaSetMonitor> m = globalRSMonitorManager.getMonitor(setName);
        if (!m) {
            LOG(0) << "failed to getMonitor of set: " << setName;
            return;
        }
    ...
    void checkSlowQueries() {
        globalConnPool.checkSlowQueries();
        shardConnectionPool.checkSlowQueries();
    }

    void checkEnableOverload() {
        globalConnPool.checkOverloadExpired();
        shardConnectionPool.checkOverloadExpired();
    }
...
bool Refresher::receivedIsMasterFromMaster(const IsMasterReply& reply) {
    ...
    LOG(0) << "Find master:" << reply.host.toString()
           << ", for set:" << _set->name;
