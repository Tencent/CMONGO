...
public:
    Geo2dFindNearCmd() : Command("geoNear", false, "superGeoNear") {}
    ...
    bool run(OperationContext* txn,
        ...
        bool superGeoNear = !cmdObj["superGeoNear"].eoo();

        LOG(1)<<"is superGeoNear:"<<superGeoNear;
        if (!cmdObj["start"].eoo()) {
		...
		if (!using2DIndex) {
            uassert(40124, "superGeoHash can only be applied on 2d index", !superGeoNear);
        }
		...
		if (superGeoNear && !cmdObj["hintScan"].eoo()) {
            uassert(40116, "hintScan must be a number", cmdObj["hintScan"].isNumber());
			nearBob.append("$hintScan", cmdObj["hintScan"].safeNumberLong());
		} else if (superGeoNear) {
			nearBob.append("$hintScan", 500);
		} else {
			nearBob.append("$hintScan", static_cast<long long int>(LONG_MAX));
		}

		if (superGeoNear && !cmdObj["hintCorrectNum"].eoo()) {
            uassert(40117, "hintCorrectNum must be a number", cmdObj["hintCorrectNum"].isNumber());
			nearBob.append("$hintCorrectNum", cmdObj["hintCorrectNum"].safeNumberLong());
		} else if (superGeoNear) {
			nearBob.append("$hintCorrectNum", 3);
		} else {
			nearBob.append("$hintCorrectNum", static_cast<long long int>(LONG_MAX));
		}
		...
		nearBob.append("$geoLimitNum", numWanted);
