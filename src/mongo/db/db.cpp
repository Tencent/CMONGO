...
static void _initAndListen(int listenPort) {
    ...
    MessageServer* server = createServer(options, std::move(handler), nullptr);
...

    {
		...
		Status status = mongo::getCMongoShardingState()->initialize(startupOpCtx.get());
		if (!status.isOK()) {
			error() << "CMongoShardingState init failed:"
				  << status.toString()
				  << endl;
			exitCleanly(EXIT_SHARDING_ERROR);
		}
