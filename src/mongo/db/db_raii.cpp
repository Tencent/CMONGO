...
AutoGetCollectionForRead::AutoGetCollectionForRead(OperationContext* txn,
    ...
    //_ensureNonBlockSnapshotIsValid(nss);
...
void AutoGetCollectionForRead::_ensureNonBlockSnapshotIsValid(const NamespaceString& nss) {
    while(true) {
        auto coll = _autoColl->getCollection();
        if (!coll) {
            return;
        ...
        auto nonBlockSnapshot = _txn->recoveryUnit()->getNonBlockReadSnaoshot();
        if (!nonBlockSnapshot) {
            return;
        }
        if (nonBlockSnapshot >= minSnapshot) {
            return;
        }
