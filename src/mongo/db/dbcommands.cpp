...
// suite to run extremely slowly. As a workaround we do not pre-allocate in Windows DEBUG builds.
    ...
    repl::ReadConcernArgs readConcernArgs;
    {
        ...
        if (replCoord->getReplicationMode() == repl::ReplicationCoordinator::Mode::modeReplSet && 
            ...
            //if (cmdName == "find" || cmdName == "getMore" || cmdName == "listDatabases" || cmdName == "listCollections" || cmdName == "listIndexes" ) {
            Status status = txn->recoveryUnit()->setReadFromNonBlockReadSnapshot();

            if (!status.isOK()) {
                if (status == ErrorCodes::ReadNonBlockNotAvailableYet) {
                    replCoord->createNonBlockSnapshot(txn);
                } else {
                    auto result = appendCommandStatus(inPlaceReplyBob, status);
                    inPlaceReplyBob.doneFast();
                    replyBuilder->setMetadata(rpc::makeEmptyMetadata());
        ...
        if (replCoord->getReplicationMode() == repl::ReplicationCoordinator::Mode::modeReplSet && replCoord->getMemberState().primary()) {
            txn->recoveryUnit()->dropNonBlockSnaphsot();
        }
    }
