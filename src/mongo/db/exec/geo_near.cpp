...
PlanStage::StageState GeoNear2DStage::initialize(OperationContext* txn,
	...
	LOG(3)<<"estimatedDistance:"<<estimatedDistance;
    if (state == PlanStage::IS_EOF) {
        ...
        if (SPHERE == _nearParams.nearQuery->centroid->crs) {
			...
			// NOTE(deyukong): if the first bike is found around 300 meter near. the boundIncrement will 
			// be 900 meter, which is too rough
			...
			_firstbound = std::min(_firstbound, kMaxEarthDistanceInMeters / 1000.0);
			// _boundsIncrement = deg2rad(estimatedDistance) * kRadiusOfEarthInMeters;
...
GeoNear2DStage::GeoNear2DStage(const GeoNearParams& nearParams,
    ...
    : NearStage(txn, kTwoDIndexNearStage.c_str(), STAGE_GEO_NEAR_2D, workingSet, collection, nearParams.nearQuery->hintScan, nearParams.nearQuery->hintCorrectNum, nearParams.nearQuery->geoLimitNum),
...
StatusWith<NearStage::CoveredInterval*>  //
	...
	bool firstCircle = _specificStats.intervalStats.empty();
    if (!_specificStats.intervalStats.empty()) {
		...
		// TODO(deyukong): 300 is meanless to mobike
	...
	double delta = _boundsIncrement;
	if (firstCircle) {
		delta = _firstbound;
	}
	LOG(3)<<"current outer"<<_currBounds.getOuter() + delta<<",lastReturned"<< (_specificStats.intervalStats.empty()?0: _specificStats.intervalStats.back().numResultsReturned);
	...
	// NOTE(deyukong): ./query/expression_index_knobs.cpp:MONGO_EXPORT_SERVER_PARAMETER(internalGeoNearQuery2DMaxCoveringCells, int, 16);
...
GeoNear2DSphereStage::GeoNear2DSphereStage(const GeoNearParams& nearParams,
    ...
    : NearStage(txn, kS2IndexNearStage.c_str(), STAGE_GEO_NEAR_2DSPHERE, workingSet, collection, nearParams.nearQuery->hintScan, nearParams.nearQuery->hintCorrectNum, nearParams.nearQuery->geoLimitNum),
      _nearParams(nearParams),
      _s2Index(s2Index),
