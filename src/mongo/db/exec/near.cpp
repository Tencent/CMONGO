...
NearStage::NearStage(OperationContext* txn,
                     ...
                     int64_t hintScan,
                     int64_t hintCorrectNum,
                     uint64_t geoLimitNum)
    : PlanStage(typeName, txn),
	  ...
	  _alreadyScanned(0),
      _hintScan(hintScan),
      _hintCorrectNum(hintCorrectNum),
      _geoLimitNum(geoLimitNum) {}
...
PlanStage::StageState NearStage::bufferNext(WorkingSetID* toReturn, Status* error) {
	...
	// covering is FetchStageWithMatch
	...
	LOG(2)<<"got result num:"<<_nextIntervalStats->numResultsBuffered
          <<",returned:"<<_alreadyReturned
          <<",scanned:"<<_alreadyScanned
          <<",_hintScan"<<_hintScan
          <<",_hintCorrectNum"<<_hintCorrectNum
          <<",_geoLimitNum"<<_geoLimitNum;

	if (_alreadyReturned < _hintCorrectNum && _alreadyScanned < _hintScan) {
		return PlanStage::NEED_TIME;
	} else if (_nextIntervalStats->numResultsBuffered >= _geoLimitNum) {
        _searchState = SearchState_Advancing;
        return PlanStage::NEED_TIME;
	}
    return PlanStage::NEED_TIME;
...
PlanStage::StageState NearStage::advanceNext(WorkingSetID* toReturn) {
    ...
    if (!_resultBuffer.empty()) {
	    ...
	    LOG(2)<<"pop item:"<<memberDistance<<" "<< _nextInterval->minDistance;
        ...
        if (inInterval || _alreadyReturned >= _hintCorrectNum || _alreadyScanned >= _hintScan) {
