...
Status setReadSnapshot(OperationContext* txn) {
    repl::ReplicationCoordinator* replCoord = repl::getGlobalReplicationCoordinator();
    if (replCoord->getReplicationMode() == repl::ReplicationCoordinator::Mode::modeReplSet && replCoord->getMemberState().secondary()) {
...
static void receivedQuery(OperationContext* txn,
    ...
    try {
        //log() << "query op:" << q.query;
        Status st = setReadSnapshot(txn);
        uassertStatusOK(st);
