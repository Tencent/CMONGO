...
bool GeoNearExpression::parseLegacyQuery(const BSONObj& obj) {
    ...
    while (it.more()) {
        ...
        } else if (equals(e.fieldName(), "$hintScan")) {
            uassert(40118, "$hintScan must be a number", e.isNumber());
			hintScan = e.safeNumberLong();
		} else if (equals(e.fieldName(), "$hintCorrectNum")) {
            uassert(40119, "$hintScan must be a number", e.isNumber());
			hintCorrectNum = e.safeNumberLong();
		} else if (equals(e.fieldName(), "$geoLimitNum")) {
            uassert(40122, "$geoLimitNum must be a number", e.isNumber());
			geoLimitNum = e.safeNumberLong();
		}
    }
...
Status GeoNearExpression::parseNewQuery(const BSONObj& obj) {
    ...
    while (it.more()) {
        ...
        if (equals(e.fieldName(), "$geometry")) {
            ...
            uassert(40120, "$hintScan must be a number", e.isNumber());
			hintScan = e.safeNumberLong();
		} else if (equals(e.fieldName(), "$hintCorrectNum")) {
            uassert(40121, "$hintScan must be a number", e.isNumber());
			hintCorrectNum = e.safeNumberLong();
		} else if (equals(e.fieldName(), "geoLimitNum")) {
			uassert(40123, "$geoLimitNum must be a number", e.isNumber());
			geoLimitNum = e.safeNumberLong();
		}
