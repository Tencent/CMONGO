...
void OpObserver::onCreateIndex(OperationContext* txn,
                               ...
                               BSONObj indexDoc,
                               bool fromMigrate) {
    repl::logOp(txn, "i", ns.c_str(), indexDoc, nullptr, nullptr, fromMigrate);
...
void OpObserver::onInserts(OperationContext* txn,
                           ...
                           bool fromMigrate) {
    repl::logOpsBatchInserts(txn, "i", nss, begin, end, fromMigrate);
...
void OpObserver::onUpdate(OperationContext* txn, oplogUpdateEntryArgs args) {
    ...
    repl::logOp(txn, "u", args.ns.c_str(), args.update, &args.criteria, &args.cmgoShardkey, args.fromMigrate);
...
OpObserver::DeleteState OpObserver::aboutToDelete(OperationContext* txn,
	...
	deleteState.cmgoShardkey = getCMongoShardingState()->getImmutableValues(ns.ns(), doc);
...
void OpObserver::onDelete(OperationContext* txn,
    ...
    repl::logOp(txn, "d", ns.ns().c_str(), deleteState.idDoc, nullptr, &deleteState.cmgoShardkey, fromMigrate);
...
void OpObserver::onOpMessage(OperationContext* txn, const BSONObj& msgObj) {
    repl::logOp(txn, "n", "", msgObj, nullptr, nullptr, false);
}
...
void OpObserver::onCreateCollection(OperationContext* txn,
    ...
    if (!collectionName.isSystemDotProfile()) {
        ...
        repl::logOp(txn, "c", dbName.c_str(), cmdObj, nullptr, nullptr, false);
    }
...
void OpObserver::onCollMod(OperationContext* txn,
    ...
    if (!NamespaceString(NamespaceString(dbName).db(), coll).isSystemDotProfile()) {
        // do not replicate system.profile modifications
        repl::logOp(txn, "c", dbName.c_str(), collModCmd, nullptr, nullptr, false);
    }
...
void OpObserver::onDropIndex(OperationContext* txn,
    ...
    repl::logOp(txn, "c", dbName.c_str(), idxDescriptor, nullptr, nullptr, false);
...
void OpObserver::onApplyOps(OperationContext* txn,
    ...
    repl::logOp(txn, "c", dbName.c_str(), applyOpCmd, nullptr, nullptr, false);
