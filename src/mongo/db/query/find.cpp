    ...
    *exhaust = false;
    ...
    if (NULL == cc) {
        ...
        *isCursorAuthorized = true;
        ...
        if (cc->isReadCommitted()) {
        ...
        } else if (cc->isReadNonBlock()) {
            auto st = txn->recoveryUnit()->setReadFromNonBlockReadSnapshot();
            if(!st.isOK()) {
                if (st == ErrorCodes::ReadNonBlockNotAvailableYet) {
            ...
            //uassertStatusOK(txn->recoveryUnit()->setReadFromNonBlockReadSnapshot());
        }
    ...
    if (shouldSaveCursor(txn, collection, state, exec.get())) {
        ...
        ClientCursor* cc =
            new ClientCursor(collection->getCursorManager(),
                             ...
                             txn->recoveryUnit()->isReadingFromNonBlockReadSnapshot(),
                             pq.getOptions(),
                             pq.getFilter());
        ccId = cc->cursorid();
