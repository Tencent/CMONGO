...
std::unique_ptr<PlanExecutor> InternalPlanner::deleteWithIndexScan(
    ...
    if (params.canonicalQuery != NULL) {
        long long deleteLimit = *(params.canonicalQuery->getParsed().getNToReturn());
        if (deleteLimit > 0)
        {
            root = stdx::make_unique<LimitStage>(txn, deleteLimit, ws.get(), root.release());
        }
    }
