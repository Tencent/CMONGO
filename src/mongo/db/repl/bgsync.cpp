...
void BackgroundSync::_produce(OperationContext* txn) {
    ...
    if (fetcherReturnStatus.code() == ErrorCodes::OplogOutOfOrder) {
        ...
        /*
         * NOTE(zhenyipeng) the default socketTimeout is 30 secs. 
         * It is too short for SC3-10G machines, so modified to 10 mins
        */
        const Seconds sc3TimeoutSeconds(600);
        const Milliseconds sc3OplogSocketTimeout(sc3TimeoutSeconds);
        auto getConnection =
            [&connection, &connectionPool, sc3OplogSocketTimeout, source]() -> DBClientBase* {
                if (!connection.get()) {
                    connection.reset(new ConnectionPool::ConnectionPtr(
                        &connectionPool, source, Date_t::now(), sc3OplogSocketTimeout));
                };
        ...
        //NOTE(zhenyipeng) create an oplog query filter
        unsigned secs = lastOpTimeFetched.getTimestamp().getSecs() + 1;
        unsigned inc = lastOpTimeFetched.getTimestamp().getInc();
        BSONObj oplogRollbackFilter = BSON("ts" << BSON("$lte" << Timestamp(secs, inc)));
        LOG(0) << "rollback queryFilter:" << oplogRollbackFilter.toString();
        ...
        _rollback(txn, source, getConnection, oplogRollbackFilter);
        stop();
...
void BackgroundSync::_fetcherCallback(const StatusWith<Fetcher::QueryResponse>& result,
    ...
    if (receivedMetadata) {
        ...
        //Just for debug , should be removed in release version
        //log() << "received metadata in queryResponse: " << sourcesLastOp;
    }
    ...
    if (sourcesLastOp < _lastOpTimeFetched) {
        //log() << "sourcesLastOpTime:"<< sourcesLastOp
        //    << "is older than _lastOpTimeFetched:" << _lastOpTimeFetched;
        sourcesLastOp = _lastOpTimeFetched;
    }
    if (_shouldChangeSyncSource(source, sourcesLastOp, syncSourceHasSyncSource)) {
        log() << "My lastAppliedOplogTime:" << lastTS.toString()
            << ", last fetched oplog time:" << _lastOpTimeFetched;
        if (retry_count_  < 5 ) {
            log() << "retry_count_: " << retry_count_<<" skipped.";
            retry_count_++;
        } else {
            log() << "retry_count_: " << retry_count_<<" failed, exceeds limit";
            retry_count_ = 0;
            return;
        }
        //return;
