...
void _logOp(OperationContext* txn,
    ...
    auto writer = _logOpWriter(txn, opstr, nss, obj, o2, sk, fromMigrate, slot.opTime, slot.hash);
    writers.emplace_back(std::move(writer));
    _logOpsInner(txn, opstr, nss, writers, fromMigrate, oplog, replMode, updateOpTime, slot.opTime);
}

void logOpsBatchInserts(OperationContext* txn,
            const char* opstr,
            const NamespaceString& nss,
    ...
    for (auto it = begin; it != end; it++) {
        ...
        auto writer = _logOpWriter(txn, opstr, nss, *it, NULL, NULL, fromMigrate, slot.opTime, slot.hash);
        writers.emplace_back(std::move(writer));
    }
...
void logOp(OperationContext* txn,
           ...
           bool fromMigrate) {
    ReplicationCoordinator::Mode replMode = ReplicationCoordinator::get(txn)->getReplicationMode();
    _logOp(txn, opstr, ns, obj, o2, sk, fromMigrate, _oplogCollectionName, replMode, true);
}
...
void SnapshotThread::run() {
    ...
    while (true) {
        ...
        for (int numSleepsDone = 0;
            ...
            //TODO(demon)
            log() << "new before clean";
            auto txn = client.makeOperationContext();
            replCoord->createNonBlockSnapshot(txn.get());
        }
        ...
        try {
            auto txn = client.makeOperationContext();
            //set internal read from snapshot
            if (replCoord->getMemberState().secondary()) {
                txn->recoveryUnit()->setReadFromNonBlockReadSnapshot();
            }
            log()<< "_readFromNonBlockReadSnapshot stats:" << txn->recoveryUnit()->isReadingFromNonBlockReadSnapshot();
            ...
            auto opTimeOfSnapshot = OpTime();
            {
                ...
                log()  << "ready to fetch a oplog, name:" << name.asU64(); 
