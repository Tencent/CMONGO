...
public:
    ...
    /**
     * Produce an iterator over oplog collection in reverse natural order in rollback
     * the filter indicates a timestamp range
     */
    virtual std::unique_ptr<Iterator> makeRollbackIterator(BSONObj filter) const = 0;
};
