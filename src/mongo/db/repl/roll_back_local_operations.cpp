...
StatusWith<RollBackLocalOperations::RollbackCommonPoint> RollBackLocalOperations::onRemoteOperation(
    ...
    if (_scanned == 0) {
        ...
        if (diff > 1800) {
            log() << "rollback too long a time period for a rollback.";
...
StatusWith<RollBackLocalOperations::RollbackCommonPoint> syncRollBackLocalOperations(
    ...
    log() << "in syncRollBackLocalOperations, begin to make an iterator by filter:" << oplogRollbackFilter.toString();
    //auto remoteIterator = remoteOplog.makeIterator();
    auto remoteIterator = remoteOplog.makeRollbackIterator(oplogRollbackFilter);
    ...
    log() << "make remote iterator succeed";
