    ...
    how.rbid = rollbackSource.getRollbackId();
    {
        ...
        try {
            ...
            auto res = syncRollBackLocalOperations(
                localOplog, rollbackSource.getOplog(), processOperationForFixUp, oplogRollbackFilter);
            if (!res.isOK()) {
...
Status syncRollback(OperationContext* txn,
    ...
    Status status = _syncRollback(txn, localOplog, rollbackSource, replCoord, sleepSecondsFn, oplogRollbackFilter);
