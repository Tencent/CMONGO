...
class ApplyOplogLock {
public:
    ApplyOplogLock(OperationContext* txn) : _txn(txn) {
        auto _manager = getGlobalServiceContext()->getGlobalStorageEngine()->getSnapshotManager();
        if (!_manager) {
            _pbwm = std::unique_ptr<Lock::ParallelBatchWriterMode>(new Lock::ParallelBatchWriterMode(_txn->lockState()));
        }
        //} else {
        //    auto replCoord = ReplicationCoordinator::get(_txn);
        //    log()<< "update NonBlockSnapshotName";
        //    replCoord->createNonBlockSnapshot(_txn);
        //}
    }
...
private:
    ...
    std::unique_ptr<Lock::ParallelBatchWriterMode> _pbwm;
...
OpTime SyncTail::multiApply(OperationContext* txn,
    ...
    uint64_t realStartTime = curTimeMicros64();
    ...
    uint64_t fsynclkTime = curTimeMicros64();

    //// stop all readers until we're done
    //Lock::ParallelBatchWriterMode pbwm(txn->lockState());
    ApplyOplogLock tmplock(txn);
    ...
    uint64_t applyOpsTime = curTimeMicros64();
    ...
    uint64_t delta = curTimeMicros64() - startTime;
    if (delta >= 30000) {
        LOG(0) << "apply oplogs cost:" << delta << " microSecons"
            << ", get fsynclk cost:" << fsynclkTime - realStartTime << " microSecons"
            << ", get ApplyOplogLock cost:" << startTime - fsynclkTime << " microSecons"
            << ", do applyOps cost:" << applyOpsTime - startTime << " microSecons"
            << ", do writeOpsToOplog cost:" << delta - (applyOpsTime - startTime) << " microSecons";
    }
    return lastOpTime;
}
