...
OpTime TopologyCoordinatorImpl::_lastSourceOpTime;
...
bool TopologyCoordinatorImpl::shouldChangeSyncSource(const HostAndPort& currentSource,
    ...
    /*if (syncSourceLastOpTime.getSecs() - 3 > _hbdata[currentSourceIndex].getAppliedOpTime().getSecs()) {
        log() << "hbdata Optime: " << _hbdata[currentSourceIndex].getAppliedOpTime()
            << " is 3 seconds older than syncSourceLastOpTime: " << syncSourceLastOpTime;
    }*/

    if (!_lastSourceOpTime.isNull()) {
        if (_lastSourceOpTime.getSecs() > currentSourceOpTime.getSecs()) {
            log() << "Fatal! currentSourceOpTime: " << currentSourceOpTime.toString()
                << " is older than lastSourceOpTime: " << _lastSourceOpTime.toString()
                << ". current syncSourceLastOpTime: " << syncSourceLastOpTime.toString()
                << ", current hbdata time:" << _hbdata[currentSourceIndex].getAppliedOpTime().toString();
            return false;
        }
    }
    _lastSourceOpTime = currentSourceOpTime;
    ...
    for (std::vector<MemberHeartbeatData>::const_iterator it = _hbdata.begin(); it != _hbdata.end();
        ...
        if (it->up() && (candidateConfig.isVoter() || !_selfConfig().isVoter()) &&
            ...
            log() << "re-evaluating sync source because our current sync source's "
                  << currentSource.toString() <<" most recent "
                  ...
                  << it->getAppliedOpTime().toString() << ". current syncSourceLastOpTime: "
                  << syncSourceLastOpTime.toString() << ". currentSource hb time: "
                  << _hbdata[currentSourceIndex].getAppliedOpTime().toString();
