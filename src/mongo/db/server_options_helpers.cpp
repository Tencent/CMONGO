...
Status addGeneralServerOptions(moe::OptionSection* options) {
    ...
    options->addOptionChaining("conf",
                               "conf,f",
                               moe::String,
                               "configuration file specifying additional options")
        .setSources(moe::SourceAllLegacy);

	options->addOptionChaining("aesEncKey", "aesEncKey", moe::String, "aes enckey")
		.setSources(moe::SourceAllLegacy);
	...
	options->addOptionChaining("logLevel",
							   "logLevel",
							   moe::String,
							   "cmongo loglevel, (info,error)->0, (debug)->1")
        .setSources(moe::SourceYAMLConfig);
	options->addOptionChaining("logPath",
							   "logPath",
							   moe::String,
							   "cmongo logPath")
		.setSources(moe::SourceYAMLConfig);

	options->addOptionChaining("metaUrl",
							   "metaUrl",
							   moe::String,
							   "cmongo etcd url")
		.setSources(moe::SourceYAMLConfig);
	options->addOptionChaining("pidFile",
							   "pidFile",
							   moe::String,
							   "cmongo pidfile")
		.setSources(moe::SourceYAMLConfig);
	options->addOptionChaining("mongo.clusterName",
							   "clusterName",
							   moe::String,
							   "cmongo mongo.clusterName")
		.setSources(moe::SourceYAMLConfig);
	options->addOptionChaining("proxy.enableSharding",
							   "enableSharding",
						       moe::Int,
							   "cmongo enable sharding")
		.setSources(moe::SourceYAMLConfig);
	options->addOptionChaining("proxy.port",
							   "proxyPort",
						       moe::Int,
							   "cmongo proxy port")
		.setSources(moe::SourceYAMLConfig);
	options->addOptionChaining("proxy.messageTimeout",
							   "messageTimeout",
						       moe::Int,
							   "cmongo proxy messageTimeout")
		.setSources(moe::SourceYAMLConfig);
	options->addOptionChaining("proxy.clientIdleTimeout",
							   "clientIdleTimeout",
						       moe::Int,
							   "cmongo proxy clientIdleTimeout")
		.setSources(moe::SourceYAMLConfig);
	options->addOptionChaining("proxy.serverIdleTimeout",
							   "serverIdleTimeout",
						       moe::Int,
							   "cmongo proxy serverIdleTimeout")
		.setSources(moe::SourceYAMLConfig);
	options->addOptionChaining("proxy.getLastErrorTimeout",
							   "getLastErrorTimeout",
						       moe::Int,
							   "cmongo proxy getLastErrorTimeout")
		.setSources(moe::SourceYAMLConfig);
	options->addOptionChaining("proxy.serverClosePoolSize",
							   "serverClosePoolSize",
						       moe::Int,
							   "cmongo proxy serverClosePoolSize")
		.setSources(moe::SourceYAMLConfig);
	options->addOptionChaining("proxy.maxPerClientConnections",
							   "maxPerClientConnections",
						       moe::Int,
							   "cmongo proxy maxPerClientConnections")
		.setSources(moe::SourceYAMLConfig);
	options->addOptionChaining("proxy.maxConnections",
							   "maxConnections",
						       moe::Int,
							   "cmongo proxy maxConnections")
		.setSources(moe::SourceYAMLConfig);
	options->addOptionChaining("asio.workerNum",
							   "",
							   moe::Int,
							   "mongos asio workerNum")
		.setSources(moe::SourceYAMLConfig);
	options->addOptionChaining("asio.minConnPerHostPerWorker",
							   "",
							   moe::Int,
							   "mongos asio minConnPerHostPerWorker")
		.setSources(moe::SourceYAMLConfig);
	options->addOptionChaining("asio.maxConnPerHostPerWorker",
							   "",
							   moe::Int,
							   "mongos asio maxConnPerHostPerWorker")
		.setSources(moe::SourceYAMLConfig);
	options->addOptionChaining("proxy.globalMaxQueueSizePerHost",
	                           "",
	                           moe::Int,
	                           "mongos waiting queue size for global pool")
        .setSources(moe::SourceYAMLConfig);
    options->addOptionChaining("proxy.shardMaxQueueSizePerHost",
                               "",
                               moe::Int,
                               "mongos waiting queue size for shard pool")
        .setSources(moe::SourceYAMLConfig);
    options->addOptionChaining("asio.maxQueueSizePerHostPerWorker",
                               "",
                               moe::Int,
                               "mongos waiting queue size for asio pool")
        .setSources(moe::SourceYAMLConfig);
    return Status::OK();
}
