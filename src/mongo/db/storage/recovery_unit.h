...
public:
    ...
    /**
     * Solve question of ParallelBatchWriterMode lock
     */
    virtual Status setReadFromNonBlockReadSnapshot() {
        return {ErrorCodes::CommandNotSupported,
        "Current storage engine done not support readNonBlockRead"};
    }

    virtual bool isReadingFromNonBlockReadSnapshot() const {
        return false;
    }

    virtual boost::optional<SnapshotName> getNonBlockReadSnaoshot() const {
        dassert(!isReadingFromNonBlockReadSnapshot());
        return {};
    }

    virtual void dropNonBlockSnaphsot() {}
