...
Status WiredTigerRecoveryUnit::setReadFromNonBlockReadSnapshot() {
    auto snapshotName = _sessionCache->snapshotManager().getNonBlockReadSnaoshot();
    if (!snapshotName) {
        return {ErrorCodes::ReadNonBlockNotAvailableYet,
                "Read non-block reads are currently not possible."};
    }
    _readFromNonBlockReadSnapshot = true;
    //check nonblocksnapshot if expire
    if (_sessionCache->snapshotManager().ifNonBlockSnapshotExpired()) {
        return {ErrorCodes::ReadNonBlockNotAvailableYet,
            "Read non-block reads are expired"};
    }
    return Status::OK();
}

boost::optional<SnapshotName> WiredTigerRecoveryUnit::getNonBlockReadSnaoshot() const {
    if (!_readFromNonBlockReadSnapshot)
        return {};
    return _sessionCache->snapshotManager().getNonBlockReadSnaoshot();
}

void WiredTigerRecoveryUnit::dropNonBlockSnaphsot() {
    _sessionCache->snapshotManager().dropNonBlockSnaphsot();
}
...
void WiredTigerRecoveryUnit::_txnOpen(OperationContext* opCtx) {
    ...
    } else if (_readFromNonBlockReadSnapshot) {
        //LOG(0) << "read from non-block snapshot";
        _sessionCache->snapshotManager().beginTransactionOnNonBlockReadSnapshot(s);
    } else {
