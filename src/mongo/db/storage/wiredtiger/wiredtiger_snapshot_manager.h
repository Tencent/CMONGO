...
public:
    ...
    void setNonBlockReadSnapshot(OperationContext* txn, const SnapshotName& name) final;
    void cleanupUnneededSnapshots() final;
    void dropAllSnapshots() final;
    Status setOplogTimeOfSnapshot(const SnapshotName& name, Timestamp ts) final;
    ...
    Status beginTransactionOnNonBlockReadSnapshot(WT_SESSION* session) const;

    boost::optional<SnapshotName> getNonBlockReadSnaoshot() const;

    void dropNonBlockSnaphsot();

    bool ifNonBlockSnapshotExpired();
...
private:
    ...
    boost::optional<SnapshotName> _nonBlockReadSnapshot;
    unsigned long long _lastNonBlockSnapshotTime = 0;
};
}
