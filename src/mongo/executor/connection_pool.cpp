...
public:
    ...
    size_t numSlowQueries(const stdx::unique_lock<stdx::mutex>& lk);
...
private:
    ...
    std::unique_ptr<TimerInterface> _overloadTimer;
    ...
    Date_t _lastCreateConnectionTime;
};

const Milliseconds ConnectionPool::kDefaultHostTimeout = Hours(24*30);
size_t const ConnectionPool::kDefaultMaxConns = std::numeric_limits<size_t>::max();
size_t const ConnectionPool::kDefaultMinConns = 1;
const Milliseconds ConnectionPool::kDefaultRefreshRequirement = Hours(6);
...
ConnectionPool::~ConnectionPool() {
    if (nullptr != _overloadTimer.get()) {
        DESTRUCTOR_GUARD(_overloadTimer->cancelTimeout();)
    }
}

Status ConnectionPool::updateConnPoolOptions(const ConnectionPool::Options& options) {
    stdx::unique_lock<stdx::mutex> lk(_mutex);
    if (options.minConnections != 0) {
        LOG(0) << "ConnectionPool minConnections change from:"
               << _options.minConnections << " to " << options.minConnections;
        _options.minConnections = options.minConnections;
    }
    if (options.maxConnections != 0) {
        LOG(0) << "ConnectionPool maxConnections change from:"
               << _options.maxConnections << " to " << options.maxConnections;
        _options.maxConnections = options.maxConnections;
    }
    if (options.overloadConfig.isOverloadEnabled() != _options.overloadConfig.isOverloadEnabled()) {
        LOG(0) << "ConnectionPool enableOverload change to:" << options.overloadConfig.isOverloadEnabled();
        _options.overloadConfig.setEnableOverload(options.overloadConfig.isOverloadEnabled());
    }
    if (options.overloadConfig.getOverloadQueueSize() > 0) {
        int oldQueueSize = _options.overloadConfig.getOverloadQueueSize();
        _options.overloadConfig.setOverloadQueueSize(options.overloadConfig.getOverloadQueueSize());
        if (oldQueueSize != _options.overloadConfig.getOverloadQueueSize()) {
            LOG(0) << "ConnectionPool overloadQueueSize change from:"
                   << oldQueueSize << " to " << _options.overloadConfig.getOverloadQueueSize();
        }
    } else {
        // Overload queue size is not set, use the normal one
        _options.overloadConfig.setOverloadQueueSize(_options.overloadConfig.getNormalQueueSize());
    }
    if (options.overloadConfig.getOverloadInUsePoolSize() > 0) {
        int oldPoolSize = _options.overloadConfig.getOverloadInUsePoolSize();
        _options.overloadConfig.setOverloadInUsePoolSize(options.overloadConfig.getOverloadInUsePoolSize());
        if (oldPoolSize != _options.overloadConfig.getOverloadInUsePoolSize()) {
            LOG(0) << "ConnectionPool overloadMaxConnecting change from:"
                   << oldPoolSize << " to " << _options.overloadConfig.getOverloadInUsePoolSize();
        }
    } else {
        // Overload pool size is not set, use the normal one
        _options.overloadConfig.setOverloadInUsePoolSize(_options.overloadConfig.getNormalInUsePoolSize());
    }
    if (options.overloadConfig.isOverloadResizing() != _options.overloadConfig.isOverloadResizing()) {
        LOG(0) << "ConnectionPool enableOverloadResizing change to:" << options.overloadConfig.isOverloadResizing();
        _options.overloadConfig.setOverloadResizing(options.overloadConfig.isOverloadResizing());
    }
    if (_options.overloadConfig.isOverloadEnabled()) {
        if (nullptr == _overloadTimer.get()) {
            _overloadTimer = std::move(_factory->makeTimer());
        }
        _overloadTimer->cancelTimeout();
        _overloadTimer->setTimeout(Seconds(10), [this](){
            checkOverload();
        });
    } else {
        if (nullptr != _overloadTimer.get()) {
...
Status ConnectionPool::getConnPoolOptions(ConnectionPool::Options& options) {
    options.minConnections = _options.minConnections;
...
void ConnectionPool::checkOverload() {
    stdx::unique_lock<stdx::mutex> lk(_mutex);

    auto currentTimestamp = _factory->now();
    if (_options.overloadConfig.isOverloadExpired(currentTimestamp)) {
        LOG(2) << "ASIO pool disable overload checking automatically after one hour";
        _options.overloadConfig.disableOverload();
    }
...
void ConnectionPool::checkSlowQueries(const stdx::unique_lock<stdx::mutex>& lk) {
    uint64_t slowCount = 0;
    uint64_t totalCount = 0;
    for (const auto& kv : _pools) {
        auto& pool = kv.second;
        totalCount += pool->inUseConnections(lk);
        slowCount += pool->numSlowQueries(lk);
    }
    int oldPoolSize = _options.overloadConfig.getOverloadInUsePoolSize();
    _options.overloadConfig.overloadResizing(_options.overloadConfig.checkOverloadSQ(slowCount, totalCount));
    if (oldPoolSize != _options.overloadConfig.getOverloadInUsePoolSize()) {
        LOG(2) << "ASIO pool in-use pool resize from " << oldPoolSize
               << " to " << _options.overloadConfig.getOverloadInUsePoolSize();
    }
}
...
size_t ConnectionPool::SpecificPool::numSlowQueries(const stdx::unique_lock<stdx::mutex>& lk) {
    size_t count = 0 ;
    auto currentTimestamp = _parent->_factory->now();
    for (auto iter = _checkedOutPool.begin(); iter != _checkedOutPool.end(); iter ++) {
        auto startTimestamp = iter->second->getStartTimestamp();
        if (_parent->_options.overloadConfig.isOverloadSlowQuery(currentTimestamp - startTimestamp)) {
            count ++;
        }
    }
    return count;
...
void ConnectionPool::SpecificPool::getConnection(const HostAndPort& hostAndPort,
                                                 ...
                                                 stdx::unique_lock<stdx::mutex> lk,
                                                 GetConnectionCallback cb) {
    if (_parent->_options.overloadConfig.isQueueOverloaded(_requests.size())) {
        log() << "ASIO pool: over loaded, current size: " << _requests.size();
        cb(Status(ErrorCodes::ServerOverloaded, "server overloaded"));
        return;
    }

    if (timeout <= Milliseconds(0) || timeout > Milliseconds(10000)) {
...
void ConnectionPool::SpecificPool::returnConnection(ConnectionInterface* connPtr,
    ...
    if (conn->getGeneration() != _generation) {
		...
		LOG(0) << "conn:" << conn->getHostAndPort() << " destroy due to mismatch generation:" << conn->getGeneration() << "," << _generation;
        return;
    }

    if (!conn->getStatus().isOK()) {
        // TODO: alert via some callback if the host is bad
		LOG(0) << "conn:" << conn->getHostAndPort() << " destroy due to status not ok";
        return;
    }
    ...
    if (needsRefreshTP <= now) {
        ...
        if (_readyPool.size() + _processingPool.size() + _checkedOutPool.size() >=
			...
			LOG(0) << "conn:" << conn->getHostAndPort() << " destroy due to expire:" << needsRefreshTP << "," << now << ","
				   << _parent->_options.refreshRequirement << "," << _parent->_options.minConnections;
            return;
        }

		LOG(0) << "conn:" << conn->getHostAndPort() << " put to _processingPool";
...
void ConnectionPool::SpecificPool::fulfillRequests(stdx::unique_lock<stdx::mutex>& lk) {
    ...
    while (_requests.size()) {
        if (_requests.top().first < currentTimestamp) {
            // Request is timeout for waiting
            auto cb = std::move(_requests.top().second);
            _requests.pop();
            lk.unlock();
            Status status(ErrorCodes::ExceededTimeLimit, "wait connection pool timeout");
            cb(status);
            lk.lock();
            log() << "dropping timeout request to " << _hostAndPort;
            continue;
        }
...
void ConnectionPool::SpecificPool::spawnConnections(stdx::unique_lock<stdx::mutex>& lk) {
    ...
    uint64_t nowCreated = _created;
    // While all of our inflight connections are less than our target
    while (openConnections(lk) < target() &&
           !_parent->_options.overloadConfig.isInUsePoolOverloaded(_processingPool.size())) {
        // 1 is mongos' default poolsize
        // make conn create gap 3 seconds, so no impulse will be transfered to mongod
        if (_parent->_options.minConnections > 1) {
            if (_lastCreateConnectionTime + Milliseconds(50) < Date_t::now()) {
                LOG(0) << "update _lastCreateConnectionTime from:" << _lastCreateConnectionTime << " to:" << Date_t::now();
                _lastCreateConnectionTime = Date_t::now();
            } else {
                break;
            }
        }
        ...
        auto handle = _parent->_factory->makeConnection(_hostAndPort, _generation);
    ...
    if (_created - nowCreated != 0) {
        LOG(0) << "host:" << _hostAndPort << ",spawnConnections," << _created - nowCreated << " will be created"
               << "_readyPoolSize:" << _readyPool.size() << ",_processingPoolSize:"
               << _processingPool.size()<<",_checkedOutPoolSize:"<<_checkedOutPool.size()
...
void ConnectionPool::SpecificPool::shutdown() {
	LOG(0) << "host:" << _hostAndPort << "shutdown";
