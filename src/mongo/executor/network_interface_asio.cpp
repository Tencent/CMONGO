...
Status NetworkInterfaceASIO::updateConnPoolOptions(const executor::ConnectionPool::Options& options) {
	return _connectionPool.updateConnPoolOptions(options);
}

Status NetworkInterfaceASIO::getConnPoolOptions(executor::ConnectionPool::Options& options) {
    return _connectionPool.getConnPoolOptions(options);
}
    ...
    auto nextStep = [this, getConnectionStartTime, cbHandle, request, onFinish](
        ...
        op->_strand.post([this, op, getConnectionStartTime] {
            ...
            if (op->_request.timeout == RemoteCommandRequest::kNoTimeout ||
                op->_request.timeout > ConnectionPool::kDefaultRefreshTimeout) {
                op->_request.timeout = ConnectionPool::kDefaultRefreshTimeout;
            }
            if (op->_request.timeout != RemoteCommandRequest::kNoTimeout) {
                ...
                op->_timeoutAlarm->asyncWait(
                    [this, op, access, generation, requestId](std::error_code ec) {
                        if (!ec) {
                            ...
                            LOG(2) << "Operation " << requestId << " timed out. request timeout " << op->_request.timeout;
