...
Status TaskExecutorPool::updateConnPoolOptions(const executor::ConnectionPool::Options& options) {
	for (auto& e : _executors) {
		Status s = e->updateConnPoolOptions(options);
		if (!s.isOK()) {
            LOG(0) << "WARN: update connection options failed, " << s.toString();
			return s;
		}
	}
	return Status::OK();
...
Status TaskExecutorPool::getConnPoolOptions(executor::ConnectionPool::Options& options) {
    executor::ConnectionPool::Options tmpOptions;
    int count = 0;

    for (auto& e : _executors) {
        Status s = e->getConnPoolOptions(tmpOptions);
        if (!s.isOK()) {
            LOG(0) << "WARN: get connection options failed, " << s.toString();
            return s;
        }
        if ((tmpOptions.minConnections != options.minConnections) 
                || (tmpOptions.maxConnections != options.maxConnections)) {
            options.minConnections = tmpOptions.minConnections;
            options.maxConnections = tmpOptions.maxConnections;
            count++;
        }
    }
    if (count > 1) {
        LOG(0) << "WARN: connection options may be not consistent";
    }
    return Status::OK();
}
