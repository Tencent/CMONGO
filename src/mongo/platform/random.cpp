...
public:
    InputStreamSecureRandom(const char* fn) :a(PseudoRandom(static_cast<int64_t>(seed.addAndFetch(1)))) {
    }
...
private:
    static AtomicInt64 seed;
    static class Initer {
        public:
            Initer(const char *fn) {
                auto seed_stream = stdx::make_unique<std::ifstream>(fn, std::ios::binary | std::ios::in);
                if (!seed_stream->is_open()) {
                    error() << "cannot open " << fn << " " << strerror(errno);
                    fassertFailed(28839);
                }

                int64_t r;
                seed_stream->read(reinterpret_cast<char*>(&r), sizeof(r));
                if (seed_stream->fail()) {
...
AtomicInt64 InputStreamSecureRandom::seed(0);
InputStreamSecureRandom::Initer InputStreamSecureRandom::_initer("/dev/urandom");
