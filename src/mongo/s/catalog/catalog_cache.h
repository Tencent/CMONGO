...
public:
     ...
     */
    virtual StatusWith<std::shared_ptr<DBConfig>> getDatabase(OperationContext* txn,
     ...
     */
    virtual void invalidate(const std::string& dbName) = 0;
...
class CatalogCacheDefault: public CatalogCache {
    MONGO_DISALLOW_COPYING(CatalogCacheDefault);
public:
	CatalogCacheDefault();
	~CatalogCacheDefault();
    ...
    virtual void invalidate(const std::string& dbName) final;
...
class CatalogCacheCMongo: public CatalogCache {
    MONGO_DISALLOW_COPYING(CatalogCacheCMongo);
public:
	CatalogCacheCMongo();
	~CatalogCacheCMongo();
    StatusWith<std::shared_ptr<DBConfig>> getDatabase(OperationContext* txn,
	...
	virtual void invalidateAll() final;
private:
    // NOTE(deyukong): cmongo uses a push-way to sync routes, so no fucking
	// cache, or it is already cached in CatalogManager
};
