...
public:
     ...
     */
    virtual Status updateShards(OperationContext *txn) = 0;
	...
	virtual StatusWith<std::shared_ptr<DBConfig>> getDbConfig(OperationContext* txn, const std::string& dbname) {
		return Status(ErrorCodes::InternalError, "CatalogManager::getDbConfig is not impl");
	}

	virtual Status syncRoutes(OperationContext* txn, const masterproto::GetClusterRoutesRsp& data) {
		return Status(ErrorCodes::InternalError, "CatalogManager::syncRoutes is not impl");
	}

	virtual Status syncRoutesRaw(OperationContext* txn, const masterproto::GetClusterRoutesRawRsp& data) {
		return Status(ErrorCodes::InternalError, "CatalogManager::syncRoutesRaw is not impl");
	}

	virtual Status dropDatabase(OperationContext *txn, const std::string& dbName) {
		return Status(ErrorCodes::InternalError, "CatalogManager::dropDatabase is not impl");
	}
