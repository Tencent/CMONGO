...
std::unique_ptr<CatalogManager> makeCatalogManager(ServiceContext* service,
    ...
    switch (configCS.type()) {
        ...
        case ConnectionString::CUSTOM: {
			// TODO(deyukong): better to pass etcdMetaUrl from function-parameters
			return stdx::make_unique<CatalogManagerCmongo>(metaUrl, clusterName, enableSharding);
...
ForwardingCatalogManager::ForwardingCatalogManager(ServiceContext* service,
    ...
    : ForwardingCatalogManager(service,
                               makeCatalogManager(service, configCS, shardRegistry, thisHost, metaUrl, clusterName, enableSharding),
                               shardRegistry,
                               thisHost,
                               metaUrl,
                               clusterName,
...
void ForwardingCatalogManager::_replaceCatalogManager(const TaskExecutor::CallbackArgs& args) {
    ...
    _actual = makeCatalogManager(_service, _nextConfigConnectionString, _shardRegistry,
								 _thisHost, _metaUrl, _clusterName, _enableSharding);
...
Status ForwardingCatalogManager::updateShards(OperationContext *txn) {
    log() << "update shards list in ForwardingCatalogManager";
    return retry(txn, [&] { return _actual->updateShards(txn); });
}
...
StatusWith<std::shared_ptr<DBConfig>> ForwardingCatalogManager::getDbConfig(OperationContext *txn, const std::string& dbname) {
    return retry(txn, [&] { return _actual->getDbConfig(txn, dbname); });
}

Status ForwardingCatalogManager::syncRoutes(OperationContext *txn, const masterproto::GetClusterRoutesRsp& data) {
    return retry(txn, [&] { return _actual->syncRoutes(txn, data); });
}

Status ForwardingCatalogManager::syncRoutesRaw(OperationContext *txn, const masterproto::GetClusterRoutesRawRsp& data) {
    return retry(txn, [&] { return _actual->syncRoutesRaw(txn, data); });
}

Status ForwardingCatalogManager::dropDatabase(OperationContext *txn, const std::string& dbName) {
    return retry(txn, [&] { return _actual->dropDatabase(txn, dbName); });
}
