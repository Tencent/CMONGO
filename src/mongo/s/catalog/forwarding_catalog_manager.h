...
public:
	...
	StatusWith<std::shared_ptr<DBConfig>> getDbConfig(OperationContext* txn, const std::string& dbname) override;

	Status syncRoutes(OperationContext* txn, const masterproto::GetClusterRoutesRsp& data) override;

	Status syncRoutesRaw(OperationContext* txn, const masterproto::GetClusterRoutesRawRsp& data) override;

	Status dropDatabase(OperationContext *txn, const std::string& dbName) override;
