...
StatusWith<ShardType> ShardType::fromPB(const masterproto::ReplicateSetInfo& rs) {
	ShardType shard;
	shard._name = rs.rsname();

	std::vector<std::string> hosts;
	for (int i = 0; i < rs.mongod_list_size(); ++i) {
		const masterproto::ContainerInfo& info = rs.mongod_list(i);
		const std::string& container_name = info.container_name();
		std::vector<std::string> container_name_parts;
		splitStringDelim(container_name, &container_name_parts, ':');
		if (container_name_parts.size() != 3) {
			return Status(ErrorCodes::InvalidReplicaSetConfig, \
				std::string("bad containername:") + container_name);
		}
		std::string host = container_name_parts[0] + ":" + container_name_parts[1];
		hosts.push_back(host);
	}
	std::string tempstr;
	joinStringDelim(hosts, &tempstr, ',');
	shard._host = rs.rsname() + "/" + tempstr;
	return shard;
}
