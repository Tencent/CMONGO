...
// NOTE(deyukong): ShouldAutoSplit should never be set in cmongo
bool Chunk::ShouldAutoSplit = false;
//bool Chunk::ShouldAutoSplit = true;

Chunk::~Chunk() = default;

ChunkDefault::ChunkDefault(OperationContext* txn, const ChunkManager* manager, const ChunkType& from)
...
ChunkDefault::ChunkDefault(const ChunkManager* info,
             const BSONObj& min,
             const BSONObj& max,
...
int ChunkDefault::mkDataWritten() {
    PseudoRandom r(static_cast<int64_t>(time(0)));
    return r.nextInt32(MaxChunkSize / ChunkManagerDefault::SplitHeuristics::splitTestFactor);
}

bool ChunkDefault::containsKey(const BSONObj& shardKey) const {
...
void ChunkDefault::pickMedianKey(OperationContext* txn, BSONObj& medianKey) const {
...
void ChunkDefault::pickSplitVector(OperationContext* txn,
...
void ChunkDefault::determineSplitPoints(OperationContext* txn,
                                 bool atMedian,
...
Status ChunkDefault::split(OperationContext* txn,
                    SplitPointMode mode,
...
Status ChunkDefault::multiSplit(OperationContext* txn, const vector<BSONObj>& m, BSONObj* res) const {
...
bool ChunkDefault::moveAndCommit(OperationContext* txn,
...
bool ChunkDefault::splitIfShould(OperationContext* txn, long dataWritten) const {
    dassert(ShouldAutoSplit);
    LastError::Disabled d(&LastError::get(cc()));

    try {
        _dataWritten += dataWritten;
        // NOTE(deyukong): it's a bad smell to use dynamic_cast here
		// But the original code use friend class to access other's private 
		// members, which is already bad
		const ChunkManagerDefault *mng =
			dynamic_cast<const ChunkManagerDefault *>(getManager());
		if(!mng) {
			LOG(0)<<"test splitIfShould on non-ChunkManagerDefault";
			return false;
	    }
        int splitThreshold = mng->getCurrentDesiredChunkSize();
        if (_minIsInf() || _maxIsInf()) {
            splitThreshold = (int)((double)splitThreshold * .9);
        }

        if (_dataWritten < splitThreshold / ChunkManagerDefault::SplitHeuristics::splitTestFactor)
            return false;

        if (!mng->_splitHeuristics._splitTickets.tryAcquire()) {
            LOG(1) << "won't auto split because not enough tickets: " << mng->getns();
            return false;
        }

        TicketHolderReleaser releaser(&(mng->_splitHeuristics._splitTickets));
...
ConnectionString ChunkDefault::_getShardConnectionString(OperationContext* txn) const {
    const auto shard = grid.shardRegistry()->getShard(txn, getShardId());
    return shard->getConnString();
}

void ChunkDefault::appendShortVersion(const char* name, BSONObjBuilder& b) const {
...
bool ChunkDefault::operator==(const Chunk& s) const {
    return getMin().woCompare(s.getMin()) == 0
		&& getMax().woCompare(s.getMax()) == 0;
}

string ChunkDefault::genID(const string& ns, const BSONObj& o) {
    StringBuilder buf;
    buf << ns << "-";
...
string ChunkDefault::toString() const {
    stringstream ss;
...
void ChunkDefault::markAsJumbo(OperationContext* txn) const {
    // set this first
...
void ChunkDefault::refreshChunkSize(OperationContext* txn) {
...
bool ChunkDefault::setMaxChunkSizeSizeMB(int newMaxChunkSize) {
...
ChunkCMongo::ChunkCMongo(const ChunkManager* info, const ShardId& shardId, uint32_t state, uint32_t chunkid)
    :_manager(info),
	 _shardId(shardId),
	 _chunkId(chunkid),
	 _state(state) {
	// NOTE(deyukong): _min and _max are meaningless in cmongo's shard system
	// but _max is used to construst ChunkMap's key
	_min = BSON("_" <<_chunkId);
	_max = BSON("_" <<_chunkId);
} 

bool ChunkCMongo::containsKey(const BSONObj& shardKey) const {
	// TODO(deyukong): implement
	return false;
}

void ChunkCMongo::appendShortVersion(const char* name, BSONObjBuilder& b) const {
    LOG(0) << "ERROR:ChunkCMongo::appendShortVersion called";
	invariant(0);
}

bool ChunkCMongo::splitIfShould(OperationContext* txn, long dataWritten) const{
    LOG(0) << "ERROR:ChunkCMongo::splitIfShould called";
	invariant(0);
	return false;
}

Status ChunkCMongo::split(OperationContext* txn,
                 SplitPointMode mode,
                 size_t* resultingSplits,
                 BSONObj* res) const {
    LOG(0) << "ChunkCMongo::split called";
	invariant(0);
	return Status::OK();
}

Status ChunkCMongo::multiSplit(OperationContext* txn,
                      const std::vector<BSONObj>& splitPoints,
                      BSONObj* res) const {
    LOG(0) << "ChunkCMongo::multiSplit called";
	invariant(0);
	return Status::OK();

}

void ChunkCMongo::pickMedianKey(OperationContext* txn, BSONObj& medianKey) const {
    LOG(0) << "ChunkCMongo::pickMedianKey called";
	invariant(0);
	return;
}

void ChunkCMongo::pickSplitVector(OperationContext* txn,
                         std::vector<BSONObj>& splitPoints,
                         long long chunkSize,
                         int maxPoints,
                         int maxObjs) const {
    LOG(0) << "ChunkCMongo::pickSplitVector called";
	invariant(0);
	return;
}

bool ChunkCMongo::moveAndCommit(OperationContext* txn,
                       const ShardId& to,
                       long long chunkSize,
                       const WriteConcernOptions* writeConcern,
                       bool waitForDelete,
                       int maxTimeMS,
                       BSONObj& res) const {
    LOG(0) << "ChunkCMongo::moveAndCommit called";
	invariant(0);
	return false;
}

void ChunkCMongo::markAsJumbo(OperationContext* txn) const {
    LOG(0) << "ChunkCMongo::markAsJumbo called";
	invariant(0);
}

std::string ChunkCMongo::toString() const {
	return "";
}

bool ChunkCMongo::operator==(const Chunk& s) const {
    LOG(0) << "ChunkCMongo::operator== called";
	invariant(0);
	return false;
}
