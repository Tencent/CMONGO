...
public:
    ...
    pair<BSONObj, shared_ptr<Chunk>> rangeFor(OperationContext* txn,
        ...
        shared_ptr<Chunk> c(new ChunkDefault(txn, _manager, chunk));
...
AtomicUInt32 ChunkManagerDefault::NextSequenceNumber(1U);
AtomicUInt32 ChunkManagerCMongo::NextSequenceNumber(1U);

ChunkManagerDefault::ChunkManagerDefault(const string& ns, const ShardKeyPattern& pattern, bool unique)
    : _ns(ns),
...
ChunkManagerDefault::ChunkManagerDefault(const CollectionType& coll)
    : _ns(coll.getNs().ns()),
...
void ChunkManagerDefault::loadExistingRanges(OperationContext* txn, const ChunkManager* oldManager) {
    int tries = 3;
...
bool ChunkManagerDefault::_load(OperationContext* txn,
                         ChunkMap& chunkMap,
                         set<ShardId>& shardIds,
                         ShardVersionMap* shardVersions,
    ...
    if (oldManager && oldManager->getVersion().isSet()) {
		...
		oldManager->GetShardVersions(shardVersions);
        ...
        for (const auto& oldChunkMapEntry : oldChunkMap) {
            ...
            shared_ptr<Chunk> newC(new ChunkDefault(
...
shared_ptr<ChunkManager> ChunkManagerDefault::reload(OperationContext* txn, bool force) const {
...
void ChunkManagerDefault::_printChunks() const {
    for (ChunkMap::const_iterator it = _chunkMap.begin(), end = _chunkMap.end(); it != end; ++it) {
        log() << *it->second;
    }
}

void ChunkManagerDefault::calcInitSplitsAndShards(OperationContext* txn,
...
Status ChunkManagerDefault::createFirstChunks(OperationContext* txn,
                                       const ShardId& primaryShardId,
                                       const vector<BSONObj>* initPoints,
                                       const set<ShardId>* initShardIds) {
    // TODO distlock?
    ...
    for (unsigned i = 0; i <= splitPoints.size(); i++) {
        ...
        ChunkType chunk;
        chunk.setName(ChunkDefault::genID(_ns, min));
        chunk.setNS(_ns);
        chunk.setMin(min);
        chunk.setMax(max);
...
ChunkPtr ChunkManagerDefault::findIntersectingChunk(OperationContext* txn, const BSONObj& shardKey) const {
    {
        BSONObj chunkMin;
        ChunkPtr chunk;
        {
...
void ChunkManagerDefault::getShardIdsForQuery(OperationContext* txn,
                                       const BSONObj& query,
    ...
    if (QueryPlannerCommon::hasNode(cq->root(), MatchExpression::GEO_NEAR)) {
        uassert(40127, "use geoNear command rather than $near query", false);
    }
...
void ChunkManagerDefault::getShardIdsForRange(set<ShardId>& shardIds,
                                       const BSONObj& min,
...
void ChunkManagerDefault::getAllShardIds(set<ShardId>* all) const {
    dassert(all);

    all->insert(_shardIds.begin(), _shardIds.end());
}

IndexBounds ChunkManagerDefault::getIndexBoundsForQuery(const BSONObj& key,
...
IndexBounds ChunkManagerDefault::collapseQuerySolution(const QuerySolutionNode* node) {
...
bool ChunkManagerDefault::compatibleWith(const ChunkManager& other, const string& shardName) const {
...
ChunkVersion ChunkManagerDefault::getVersion(const std::string& shardName) const {
...
ChunkVersion ChunkManagerDefault::getVersion() const {
    return _version;
}

string ChunkManagerDefault::toString() const {
    StringBuilder sb;
...
int ChunkManagerDefault::getCurrentDesiredChunkSize() const {
...
repl::OpTime ChunkManagerDefault::getConfigOpTime() const {
    return _configOpTime;
}

void ChunkManagerDefault::GetShardVersions(ShardVersionMap* out) const {
	*out = _shardVersions;
}

ChunkManagerCMongo::ChunkManagerCMongo(const string& ns, const ShardKeyPattern& pattern)
      :_ns(ns),
	   _keyPattern(pattern.getKeyPattern()),
	   _unique(false),
	   _sequenceNumber(NextSequenceNumber.addAndFetch(1)) {
	// TODO(deyukong): what to do with unique?
}

Status ChunkManagerCMongo::createFirstChunks(OperationContext* txn,
                             const ShardId& primaryShardId,
                             const std::vector<BSONObj>* initPoints,
                             const std::set<ShardId>* initShardIds) {
	LOG(0)<< "ERROR:ChunkManagerCMongo::createFirstChunks called";
	invariant(0);
	return Status::OK();
}

void ChunkManagerCMongo::loadExistingRanges(OperationContext* txn, const ChunkManager* oldManager){
	LOG(0)<< "ERROR:ChunkManagerCMongo::loadExistingRanges called";
	invariant(0);
}

void ChunkManagerCMongo::calcInitSplitsAndShards(OperationContext* txn,
                                 const ShardId& primaryShardId,
                                 const std::vector<BSONObj>* initPoints,
                                 const std::set<ShardId>* initShardIds,
                                 std::vector<BSONObj>* splitPoints,
                                 std::vector<ShardId>* shardIds) const {
	LOG(0)<< "ERROR:ChunkManagerCMongo::calcInitSplitsAndShards called";
	invariant(0);
}

ChunkPtr ChunkManagerCMongo::findIntersectingChunk(OperationContext* txn, const BSONObj& shardKey) const {
	StatusWith<uint32_t> chunkid = cmongo::calcChunkId(shardKey);
	uassertStatusOK(chunkid.getStatus());
	BSONObj key = BSON("_"<<chunkid.getValue());
	ChunkMap::const_iterator it = _chunkMap.find(key);
	invariant(it != _chunkMap.end());
	return it->second;
}

void ChunkManagerCMongo::getShardIdsForQuery(OperationContext* txn,
                             const BSONObj& query,
    ...
    auto shardKeyToFind = _keyPattern.extractShardKeyFromQuery(*cq);
    if (shardKeyToFind.isOK() && !shardKeyToFind.getValue().isEmpty()) {
		auto shardkeySizeStatus = ShardKeyPattern::checkShardKeySize(shardKeyToFind.getValue());
		if (!shardkeySizeStatus.isOK()) {
			LOG(0) << query << " has invalid shardkey length";
			return;
		}
        auto chunk = findIntersectingChunk(txn, shardKeyToFind.getValue());
        shardIds->insert(chunk->getShardId());
...
void ChunkManagerCMongo::getAllShardIds(std::set<ShardId>* all) const {
    all->insert(_shardIds.begin(), _shardIds.end());
}

void ChunkManagerCMongo::getShardIdsForRange(std::set<ShardId>& shardIds,
                             const BSONObj& min,
                             const BSONObj& max) const {
    shardIds.insert(_shardIds.begin(), _shardIds.end());
}

IndexBounds ChunkManagerCMongo::getIndexBoundsForQuery(const BSONObj& key,
                                              const CanonicalQuery& canonicalQuery) {
	LOG(0)<<"ERROR:ChunkManagerCMongo::getIndexBoundsForQuery called";
	invariant(0);
    IndexBounds bounds;
	return bounds;
}

IndexBounds ChunkManagerCMongo::collapseQuerySolution(const QuerySolutionNode* node) {
	LOG(0)<<"ERROR:ChunkManagerCMongo::collapseQuerySolution called";
	invariant(0);
    IndexBounds bounds;
	return bounds;
}

bool ChunkManagerCMongo::compatibleWith(const ChunkManager& other, const std::string& shard) const {
	// NOTE(deyukong): maybe not the best way
	return true;
}

std::string ChunkManagerCMongo::toString() const {
	// TODO(deyukong): implement
	return "";
}

ChunkVersion ChunkManagerCMongo::getVersion(const std::string& shardName) const {
	return ChunkVersion(0, 0, OID());
}

ChunkVersion ChunkManagerCMongo::getVersion() const {
	return ChunkVersion(0, 0, OID());
}

void ChunkManagerCMongo::_printChunks() const {
	// TODO(deyukong): implement
	return;
}

int ChunkManagerCMongo::getCurrentDesiredChunkSize() const {
	LOG(0)<<"ERROR:ChunkManagerCMongo::getCurrentDesiredChunkSize called";
	invariant(0);
	return 0;
}

std::shared_ptr<ChunkManager> ChunkManagerCMongo::reload(OperationContext* txn,
                                         bool force) const {
	LOG(0)<<"ERROR:ChunkManagerCMongo::reload called";
	invariant(0);
...
repl::OpTime ChunkManagerCMongo::getConfigOpTime() const {
	LOG(0)<<"ERROR:ChunkManagerCMongo::getConfigOpTime called";
	invariant(0);
	return _configOpTime;
}

void ChunkManagerCMongo::GetShardVersions(ShardVersionMap *out) const {
	LOG(0)<<"ERROR:ChunkManagerCMongo::GetShardVersions called";
	invariant(0);
}

Status ChunkManagerCMongo::parseFromPB(const masterproto::TableRoutes& pb) {
	_chunkMap.clear();
	_shardIds.clear();
	for (int i = 0; i < pb.routes_size(); ++i) {
		const masterproto::RouteChunk& pb_chunk = pb.routes(i);
        shared_ptr<ChunkCMongo> c(new ChunkCMongo(this, pb_chunk.shard_name(), pb_chunk.state(), i));
        _chunkMap.insert(make_pair(c->getMax(), c));
		_shardIds.insert(pb_chunk.shard_name());
	}
	return Status::OK();
} 
