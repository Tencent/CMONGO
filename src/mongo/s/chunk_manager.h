...
public:
    ...
    virtual const std::string& getns() const = 0;

    virtual const ShardKeyPattern& getShardKeyPattern() const = 0;
     ...
     * has been updated
     */
    virtual unsigned long long getSequenceNumber() const = 0;
    ...
    virtual Status createFirstChunks(OperationContext* txn,
                             const ShardId& primaryShardId,
                             const std::vector<BSONObj>* initPoints,
                             const std::set<ShardId>* initShardIds) = 0;

    // Loads existing ranges based on info in chunk manager
    virtual void loadExistingRanges(OperationContext* txn, const ChunkManager* oldManager) = 0;


    // Helpers for load
    virtual void calcInitSplitsAndShards(OperationContext* txn,
                                 const ShardId& primaryShardId,
                                 const std::vector<BSONObj>* initPoints,
                                 const std::set<ShardId>* initShardIds,
                                 std::vector<BSONObj>* splitPoints,
                                 std::vector<ShardId>* shardIds) const = 0;
     ...
     */
    virtual ChunkPtr findIntersectingChunk(OperationContext* txn, const BSONObj& shardKey) const = 0;

    virtual void getShardIdsForQuery(OperationContext* txn,
                             const BSONObj& query,
                             std::set<ShardId>* shardIds) const = 0;

    virtual void getAllShardIds(std::set<ShardId>* all) const = 0;

    /** @param shardIds set to the shard ids for shards
     *         covered by the interval [min, max], see SERVER-4791
     */
    virtual void getShardIdsForRange(std::set<ShardId>& shardIds,
                             const BSONObj& min,
                             const BSONObj& max) const = 0;

    virtual const ChunkMap& getChunkMap() const = 0;

    /**
     * Returns true if, for this shard, the chunks are identical in both chunk managers
     */
    virtual bool compatibleWith(const ChunkManager& other, const std::string& shard) const = 0;

    virtual std::string toString() const = 0;

    virtual ChunkVersion getVersion(const std::string& shardName) const = 0;
    virtual ChunkVersion getVersion() const = 0;

    virtual void _printChunks() const = 0;

    virtual int getCurrentDesiredChunkSize() const = 0;

    virtual std::shared_ptr<ChunkManager> reload(OperationContext* txn,
                                         bool force = true) const = 0;  // doesn't modify self!
	...
	virtual void GetShardVersions(ShardVersionMap* out) const = 0;
};
...
*/
class ChunkManagerDefault: public ChunkManager {
public:

    // Loads a new chunk manager from a collection document
    explicit ChunkManagerDefault(const CollectionType& coll);

    // Creates an empty chunk manager for the namespace
    ChunkManagerDefault(const std::string& ns, const ShardKeyPattern& pattern, bool unique);

    const std::string& getns() const final {
        return _ns;
    }
    const ShardKeyPattern& getShardKeyPattern() const final {
        return _keyPattern;
    }
     ...
     * has been updated
     */
    unsigned long long getSequenceNumber() const final {
    ...
    Status createFirstChunks(OperationContext* txn,
                             const ShardId& primaryShardId,
                             const std::vector<BSONObj>* initPoints,
                             const std::set<ShardId>* initShardIds) final;

    // Loads existing ranges based on info in chunk manager
    void loadExistingRanges(OperationContext* txn, const ChunkManager* oldManager) final;
    ...
    void calcInitSplitsAndShards(OperationContext* txn,
                                 ...
                                 const std::set<ShardId>* initShardIds,
                                 std::vector<BSONObj>* splitPoints,
                                 std::vector<ShardId>* shardIds) const final;

    //
    // Methods to use once loaded / created
    //

    int numChunks() const final {
     ...
     *  findIntersectingChunk() on {a : hash("foo") }
     */
    ChunkPtr findIntersectingChunk(OperationContext* txn, const BSONObj& shardKey) const final;

    void getShardIdsForQuery(OperationContext* txn,
                             const BSONObj& query,
                             std::set<ShardId>* shardIds) const final;

    void getAllShardIds(std::set<ShardId>* all) const final;
    ...
    const ChunkMap& getChunkMap() const final {
        return _chunkMap;
    }
     ...
     * Returns true if, for this shard, the chunks are identical in both chunk managers
     */
    bool compatibleWith(const ChunkManager& other, const std::string& shard) const final;

    std::string toString() const final;

    ChunkVersion getVersion(const std::string& shardName) const final;
    ChunkVersion getVersion() const final;

    void _printChunks() const final;

    int getCurrentDesiredChunkSize() const final;

    std::shared_ptr<ChunkManager> reload(OperationContext* txn,
                                         bool force = true) const final;  // doesn't modify self!
	...
	void GetShardVersions(ShardVersionMap *out) const final;
...
class ChunkManagerCMongo: public ChunkManager {
public:
    ChunkManagerCMongo(const std::string& ns, const ShardKeyPattern& pattern);
