...
Status ShardRegistry::updateConnPoolOptions(const executor::ConnectionPool::Options& options) {
	if (_executorPool.get() == nullptr) {
        return Status(ErrorCodes::InternalError, "updateConnPoolOptions _executorPooli null");
	}
	return _executorPool->updateConnPoolOptions(options);
}

Status ShardRegistry::getConnPoolOptions(executor::ConnectionPool::Options& options) {
    if (_executorPool.get() == nullptr) {
        return Status(ErrorCodes::InternalError, "getConnPoolOptions _executor null");
    }
    return _executorPool->getConnPoolOptions(options);
}
