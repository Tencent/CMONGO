...
public:
    ...
    bool run(OperationContext* opCtx,
		...
		const auto& rolename = args.roleName.getFullName();
		if (rolename.find("restoreoplog") != std::string::npos) {
			LOG(0) << "ERROR: try to update restoreoplog role from mongos:" << rolename;
			return CommandHelpers::appendCommandStatus(result, Status(ErrorCodes::BadValue, "bad rolename"));
		}
...
public:
    ...
    bool run(OperationContext* opCtx,
		...
		if (roleName.getFullName().find("restoreoplog") != std::string::npos) {
			LOG(0) << "ERROR: try to drop restoreoplog role from mongos:" << roleName;
			return CommandHelpers::appendCommandStatus(result, Status(ErrorCodes::BadValue, "bad rolename"));
		}
