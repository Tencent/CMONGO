...
public:
    ...
    virtual bool isProxyCommand() const final {
        return false;
    }
    ...
    bool enhancedRun(OperationContext* opCtx,
        ...
        if (db == NamespaceString::kAdminDb || db == NamespaceString::kConfigDb) {
			return CommandHelpers::appendCommandStatus(result, Status(ErrorCodes::InvalidNamespace, "do not allow write on admin or config"));
        }
		...
		// TODO(deyukong): make sure this ns collname is not $cmd
		auto nss = batchedRequest.getNS();
        if (grid.getFlowManager()->hitFlowControlForWrite(opCtx, nss) &&
            _writeType != BatchedCommandRequest::BatchType_Delete) {
            std::string stat = grid.getFlowManager()->getGlobalStat(opCtx);
            std::stringstream reason;
            reason << "write is banned due to " << stat;
            return CommandHelpers::appendCommandStatus(result, Status(ErrorCodes::FlowControlForWrite, reason.str()));
        }

        BatchWriteExecStats stats;
        BatchedCommandResponse response;

        std::unique_ptr<proxy_stats::RequestStats> proxy_stats;
        if (_writeType == BatchedCommandRequest::BatchType_Insert) {
            proxy_stats = std::move(stdx::make_unique<proxy_stats::InsertStats>(nss));
        } else if (_writeType == BatchedCommandRequest::BatchType_Update) {
            proxy_stats = std::move(stdx::make_unique<proxy_stats::UpdateStats>(nss));
        } else if (_writeType == BatchedCommandRequest::BatchType_Delete) {
            proxy_stats = std::move(stdx::make_unique<proxy_stats::DeleteStats>(nss));
        }
