...
public:
    ...
    virtual bool run(OperationContext* txn,
		...
		if (dbname == "admin" || dbname == "local") {
            return appendCommandStatus(result, Status(ErrorCodes::InvalidNamespace, "do not allow write on admin or local"));
		}
        BatchedCommandRequest request(_writeType);
        BatchedCommandResponse response;
        auto nss = NamespaceString(parseNs(dbname, cmdObj));

        if (grid.getFlowManager()->hitFlowControlForWrite(txn, nss) &&
            _writeType != BatchedCommandRequest::BatchType_Delete) {
			std::string stat = grid.getFlowManager()->getGlobalStat(txn);
			stringstream reason;
			reason << "write is banned due to " << stat;
            return appendCommandStatus(result, Status(ErrorCodes::FlowControlForWrite, reason.str()));
        }
		...
		if (_writeType == BatchedCommandRequest::BatchType_Insert) {
			stats = std::move(stdx::make_unique<proxy_stats::InsertStats>(nss));
        } else if (_writeType == BatchedCommandRequest::BatchType_Update) {
			stats = std::move(stdx::make_unique<proxy_stats::UpdateStats>(nss));
        } else if (_writeType == BatchedCommandRequest::BatchType_Delete) {
			stats = std::move(stdx::make_unique<proxy_stats::DeleteStats>(nss));
        }
		...
		if (delta >= 100000) {
			// LOG(0)<<"slow query:"<< cmdObj << " " << delta << " microSeconds";
		}
