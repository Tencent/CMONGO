...
public:
    ...
    bool run(OperationContext* txn,
             ...
             string& errmsg,
             BSONObjBuilder& result) {
		StatusWith<std::shared_ptr<DBConfig>> dbcfg = grid.catalogManager(txn)->getDbConfig(txn, dbName);
		if (!dbcfg.getStatus().isOK()) {
			log() << "create command;" << cmdObj << " failed with status:" << dbcfg.getStatus();
			return  appendCommandStatus(result, dbcfg.getStatus());
		}
		if (dbcfg.getValue()->isShardingEnabled()) {
			return  appendCommandStatus(result, Status(ErrorCodes::IllegalOperation, "sharded db can not explictly create table"));
		} else {
			return passthrough(txn, dbcfg.getValue(), cmdObj, result);
		}
    }
} createCmd;
...
public:
    ...
    bool run(OperationContext* txn,
        ...
        if (dbName == "config" || dbName == "admin" || dbName == "local") {
            return appendCommandStatus(
                result, Status(ErrorCodes::IllegalOperation, "Cannot drop tables in config or admin or local"));
        }
...
public:
    ...
    bool run(OperationContext* txn,
        ...
        if (dbNameFrom == "admin" || dbNameFrom == "local" ||
            dbNameTo == "admin" || dbNameTo == "local") {
            return appendCommandStatus(result,
                    Status(ErrorCodes::InvalidNamespace,
                           "do not allow renameCollection on admin or local"));
        }
...
public:
    Geo2dFindNearCmd() : PublicGridCommand("geoNear", "superGeoNear") {}
...
class PtGeo2dFindNearCmd : public PublicGridCommand {
public:
	PtGeo2dFindNearCmd() : PublicGridCommand("ptGeoNear") {}
	virtual bool passOptions() const {
		return true;
	}
    virtual void addRequiredPrivileges(const std::string& dbname,
	...
	static inline bool bsonToCoord(const BSONElement& ele, double* lon, double* lat) {
		if (ele.type() != Array) {
			return false;
		}
		vector<BSONElement> center_array = ele.Array();
		if ((center_array).size() != 2) {
			return false;
		}
		if (!center_array[0].isNumber() || !center_array[1].isNumber()) {
			return false;
		}
		*lon = center_array[0].numberDouble();
		*lat = center_array[1].numberDouble();
		return true;
	}

	static inline double radians(double x) {
    ...
    bool run(OperationContext* txn,
		...
		newCmd.append("find", nss.coll());
		BSONObj query = getQuery(cmdObj);

		if (cmdObj["filter"].eoo()) {
			errmsg = "ptGeoNear should have a filter";
			return false;
		}
		newCmd.append(cmdObj["filter"]);

		double lon = 0;
		double lat = 0;
		if (cmdObj["center"].eoo()) {
			errmsg = "please pass center, it's complex to extract filter params";
			return false;
		}
		BSONElement center = cmdObj["center"];
		if (!bsonToCoord(center, &lon, &lat)) {
			errmsg = "bad center format";
			return false;
		}

		if (!cmdObj["projection"].eoo()) {
			newCmd.append(cmdObj["projection"]);
		}

		if (!cmdObj["batchSize"].eoo()) {
			newCmd.append(cmdObj["batchSize"]);
		}

		if (cmdObj["singleBatch"].eoo()) {
			newCmd.append("singleBatch", true);
		} else if (!cmdObj["singleBatch"].isBoolean()) {
			errmsg = "singleBatch should be true|false";
			return false;
		} else if (!cmdObj["singleBatch"].boolean()) {
			errmsg = "singleBatch should only be true";
			return false;
		} else {
			newCmd.append("singleBatch", true);
		}
		...
		newCmd.append("limit", limit);
		BSONObj newCmdObj = newCmd.done();

        auto conf = uassertStatusOK(grid.catalogCache()->getDatabase(txn, dbName));
        if (!conf->isShardingEnabled() || !conf->isSharded(fullns)) {
			errmsg = "please use normal find + $near on non-shard clusters";
			return false;
        }
        ...
        for (const ShardId& shardId : shardIds) {
            ...
            futures.push_back(
                Future::spawnCommand(shard->getConnString().toString(), dbName, newCmdObj, options));
            shardArray.append(shardId);
        }
        ...
        for (list<shared_ptr<Future::CommandResult>>::iterator i = futures.begin();
			...
			if (!res->result().hasField("cursor")) {
				errmsg = "internal bad return format";
				return false;
			}
			BSONObj firstCursor = res->result()["cursor"].embeddedObjectUserCheck();
			if (!firstCursor.hasField("firstBatch")) {
				errmsg = "internal bad return format";
				return false;
			}
			BSONObj firstBatch = firstCursor["firstBatch"].embeddedObjectUserCheck();
			BSONForEach(e, firstBatch) {
				if (!e.embeddedObjectUserCheck().hasField("coordinate")) {
					errmsg = "geoField must be named 'coordinate'";
					return false;
				}
				if (!e["coordinate"].embeddedObjectUserCheck().hasField("coordinates")) {
					errmsg = "coordinate has no subfield 'coordinates'";
					return false;
				}
				double lon1, lat1;
				if (!bsonToCoord(e["coordinate"]["coordinates"], &lon1, &lat1)) {
					errmsg = "bad internal coord format";
					return false;
				}
				double dis = distance(lat, lon, lat1, lon1);
				BSONObjBuilder bb;
				bb.appendElements(e.embeddedObjectUserCheck()).append("distance", dis);
				results.insert(make_pair(dis, bb.obj()));
			}
		}
