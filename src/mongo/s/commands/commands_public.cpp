...
public:
    ...
    bool run(OperationContext* opCtx,
        ...
        uassert(ErrorCodes::InvalidNamespace,
                ...
                fullnsFrom.isValid());
        uassert(ErrorCodes::InvalidNamespace,
                "do not allow renameCollection on admin, local or config",
                (fullnsFrom.db() != NamespaceString::kAdminDb &&
                 fullnsFrom.db() != NamespaceString::kLocalDb &&
                 fullnsFrom.db() != NamespaceString::kConfigDb));
        ...
        uassert(ErrorCodes::InvalidNamespace,
                "do not allow renameCollection on admin, local or config",
                (fullnsTo.db() != NamespaceString::kAdminDb &&
                 fullnsTo.db() != NamespaceString::kLocalDb &&
                 fullnsTo.db() != NamespaceString::kConfigDb));
...
public:
    ...
    bool run(OperationContext* opCtx,
        ...
        if (!fromhost.empty()) {
			return CommandHelpers::appendCommandStatus(result, Status(ErrorCodes::BadValue, "can only copydb from the same cluster"));
            //return passthrough(
...
class Geo2dSearchCmd: public PublicGridCommand {
public:
	Geo2dSearchCmd(): PublicGridCommand("geoSearch") {}

	std::string help() const override {
		return "http://dochub.mongodb.org/core/geo";
	}
    ...
    bool run(OperationContext* opCtx,
        ...
        if (!routingInfo.cm()) {
	        ...
	        return CommandHelpers::appendCommandStatus(
				result, {ErrorCodes::InvalidNamespace, "not supported on sharded tables"});
		}
	}
} geo2dSearchCmd;
...
public:
    ...
    bool run(OperationContext* opCtx,
        ...
        if (dbName == NamespaceString::kLocalDb) {
            ...
            if (shardIds.size() == 1) {
                log()<<"db:"<<dbName<<" ,cmd:"<<cmdObj.toString();
                //get primary shard
                const auto shardStatus = grid.shardRegistry()->getShard(opCtx, shardIds[0]);
                if (!shardStatus.isOK()) {
                    //return false;
                    return appendEmptyResultSet(result, shardStatus.getStatus(), nss.ns());
                }
                const auto s = shardStatus.getValue();
                auto commandResponse = uassertStatusOK(s->runCommandWithFixedRetryAttempts(opCtx,
                ...
                //        CommandHelpers::filterCommandReplyForPassthrough(std::move(commandResponse.response)));
                BSONObj x = std::move(commandResponse.response);
                result.appendElements(x);
