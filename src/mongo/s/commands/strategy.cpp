...
void execCommandClient(OperationContext* opCtx,
	...
	std::unique_ptr<proxy_stats::RequestStats> stats;
...
//            dbname != NamespaceString::kLocalDb);
    ...
    if (c->isProxyCommand()) {
        NamespaceString nss(dbname, "$cmd");
        stats = std::move(stdx::make_unique<proxy_stats::CommandStats>(nss));
    }
...
bool runCommand(OperationContext* opCtx, const OpMsgRequest& request, BSONObjBuilder&& builder) {
    ...
    if (!command) {
        ...
        return isProxyCommand;
    }
    isProxyCommand = command->isProxyCommand();
...
DbResponse Strategy::queryOp(OperationContext* opCtx, const NamespaceString& nss, DbMessage* dbm) {
	...
	proxy_stats::ReadStats stats(NamespaceString(q.ns));
...
DbResponse Strategy::clientCommand(OperationContext* opCtx, const Message& m) {
    ...
    [&] {
        ...
        try {  // Parse.
            ...
            collection = request.getCommandField().toString();
            LOG(3) << "request: " << request.getCommandName().toString()
        ...
        try {  // Execute.
            ...
            isCommand = runCommand(opCtx, request, reply->getInPlaceReplyBuilder(0));
        ...
        NamespaceString nss(db, isCommand ? std::string("$cmd") : collection);
        if (isSuccess) {
            // Save the statistics for success
            grid.getFlowManager()->onSuccesses(nss);
        } else {
            // Save the statistics for server error
...
DbResponse Strategy::getMore(OperationContext* opCtx, const NamespaceString& nss, DbMessage* dbm) {
	...
	proxy_stats::ReadStats stats(nss);
