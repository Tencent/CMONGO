...
CollectionInfoDefault::~CollectionInfoDefault() = default;

CollectionInfoDefault::CollectionInfoDefault() {
	_dirty = false;
	_dropped = false;
}

CollectionInfoDefault::CollectionInfoDefault(OperationContext* txn,
    ...
    std::unique_ptr<ChunkManager> manager(stdx::make_unique<ChunkManagerDefault>(coll));
...
void CollectionInfoDefault::resetCM(ChunkManager* cm) {
    invariant(cm);
    invariant(_cm);
...
void CollectionInfoDefault::unshard() {
    _cm.reset();
    _dropped = true;
    _dirty = true;
    _key = BSONObj();
...
void CollectionInfoDefault::useChunkManager(std::shared_ptr<ChunkManager> manager) {
    _cm = manager;
...
void CollectionInfoDefault::save(OperationContext* txn, const string& ns) {
    CollectionType coll;
...
CollectionInfoCMongo::CollectionInfoCMongo() = default;
CollectionInfoCMongo::~CollectionInfoCMongo() = default;

void CollectionInfoCMongo::resetCM(ChunkManager *cm) {
	LOG(0)<<"ERROR:CollectionInfoCMongo::resetCM called";
	invariant(0);
}

void CollectionInfoCMongo::unshard() {
	_cm.reset();
    _key = BSONObj();
}

void CollectionInfoCMongo::save(OperationContext* txn, const std::string& ns) {
	LOG(0)<<"ERROR:CollectionInfoCMongo::save called";
	invariant(0);
}

void CollectionInfoCMongo::useChunkManager(std::shared_ptr<ChunkManager> manager) {
	_cm = manager;
    _key = manager->getShardKeyPattern().toBSON().getOwned();
}

bool CollectionInfoCMongo::unique() const {
	// TODO(deyukong): what to do, etcd does not have this
	return false;
}

Status CollectionInfoCMongo::parseFromPB(const masterproto::TableRoutes& pb) {
	_ns = NamespaceString(pb.ns());
	_routeMd5 = pb.md5();
	BSONObjBuilder b;
	for(int i = 0; i < pb.shard_key_size(); i++) {
		b.append(pb.shard_key(i), 1);
	}
	ShardKeyPattern pattern(b.obj());
    std::unique_ptr<ChunkManagerCMongo> manager(stdx::make_unique<ChunkManagerCMongo>(_ns.toString(), pattern));
	Status s = manager->parseFromPB(pb);
	if (!s.isOK()) {
		LOG(0)<<"ERROR:ChunkManagerCMongo::parseFromPB failed:"<<s;
	}
    useChunkManager(std::move(manager));
	return s;
}
...
DBConfigDefault::DBConfigDefault(std::string name, const DatabaseType& dbt, repl::OpTime configOpTime)
...
DBConfigDefault::~DBConfigDefault() = default;

bool DBConfigDefault::isSharded(const string& ns) {
...
const ShardId& DBConfigDefault::getShardId(OperationContext* txn, const string& ns) {
...
void DBConfigDefault::invalidateNs(const std::string& ns) {
    stdx::lock_guard<stdx::mutex> lk(_lock);
...
void DBConfigDefault::enableSharding(OperationContext* txn) {
...
bool DBConfigDefault::removeSharding(OperationContext* txn, const string& ns) {
    if (!_shardingEnabled) {
...
// shard
void DBConfigDefault::getChunkManagerOrPrimary(OperationContext* txn,
                                        const string& ns,
...
std::shared_ptr<ChunkManager> DBConfigDefault::getChunkManagerIfExists(OperationContext* txn,
                                                                const string& ns,
                                                                bool shouldReload,
                                                                bool forceReload) {
...
std::shared_ptr<ChunkManager> DBConfigDefault::getChunkManager(OperationContext* txn,
                                                        const string& ns,
                                                        bool shouldReload,
                                                        bool forceReload) {
    BSONObj key;
...

    {
        ...
        tempChunkManager.reset(new ChunkManagerDefault(
...
void DBConfigDefault::setPrimary(OperationContext* txn, const ShardId& newPrimaryId) {
    stdx::lock_guard<stdx::mutex> lk(_lock);
    _primaryId = newPrimaryId;
    _save(txn);
}

bool DBConfigDefault::load(OperationContext* txn) {
    const auto currentReloadIteration = _reloadCount.load();
    stdx::lock_guard<stdx::mutex> lk(_lock);
    return _loadIfNeeded(txn, currentReloadIteration);
}

bool DBConfigDefault::_loadIfNeeded(OperationContext* txn, Counter reloadIteration) {
    ...
    for (const auto& coll : collections) {
        ...
        if (coll.getDropped()) {
            ...
            _collections[coll.getNs().ns()] =
                CollectionInfoDefault(txn, coll, configOpTimeWhenLoadingColl);
            numCollsSharded++;
        }
    }
...
void DBConfigDefault::_save(OperationContext* txn, bool db, bool coll) {
    if (db) {
        DatabaseType dbt;
        dbt.setName(_name);
...
bool DBConfigDefault::reload(OperationContext* txn) {
...
bool DBConfigDefault::dropDatabase(OperationContext* txn, string& errmsg) {
    /**
...
bool DBConfigDefault::_dropShardedCollections(OperationContext* txn,
                                       int& num,
                                       set<ShardId>& shardIds,
                                       string& errmsg) {
    num = 0;
...
void DBConfigDefault::getAllShardIds(set<ShardId>* shardIds) {
    dassert(shardIds);
...
void DBConfigDefault::getAllShardedCollections(set<string>& namespaces) {
    stdx::lock_guard<stdx::mutex> lk(_lock);
...
DBConfigCMongo::~DBConfigCMongo() = default;
DBConfigCMongo::DBConfigCMongo(const std::string& name, const std::string& primaryShard, bool shardingEnabled)
	    : _name(name),
		  _primaryId(primaryShard),
		  _shardingEnabled(shardingEnabled) {
}

void DBConfigCMongo::invalidateNs(const std::string& ns) {
	LOG(0)<<"DBConfigCMongo::invalidateNs callled, and does nothing";
}

void DBConfigCMongo::enableSharding(OperationContext* txn) {
	LOG(0)<<"DBConfigCMongo::enableSharding called with ns:"<<_name;
	// NOTE(deyukong): cmongo sharding-cluster allows sharding by default
	// if it is cmongo none-cluster, this function should never be callled here
	invariant(_shardingEnabled);
	return;
}

bool DBConfigCMongo::removeSharding(OperationContext* txn, const std::string& ns) {
	LOG(0)<<"DBConfigCMongo::removeSharding should never be called";
	invariant(0);
	return false;
}

bool DBConfigCMongo::isSharded(const std::string& ns) {
    stdx::lock_guard<stdx::mutex> lk(_lock);
...
void DBConfigCMongo::getChunkManagerOrPrimary(OperationContext* txn,
                                  const std::string& ns,
                                  std::shared_ptr<ChunkManager>& manager,
                                  std::shared_ptr<Shard>& primary) {
	// TODO(deyukong): what to do with non-shard table in shard-cluster
    stdx::lock_guard<stdx::mutex> lk(_lock);
    manager.reset();
    primary.reset();
	if (!_shardingEnabled) {
		...
		std::unique_ptr<CollectionInfoCMongo>& cInfo = i->second;
		manager = cInfo->getCM();
	}
}

std::shared_ptr<ChunkManager> DBConfigCMongo::getChunkManager(OperationContext* txn,
                                                  const std::string& ns,
                                                  bool reload,
                                                  bool forceReload) {
    stdx::lock_guard<stdx::mutex> lk(_lock);
...
std::shared_ptr<ChunkManager> DBConfigCMongo::getChunkManagerIfExists(OperationContext* txn,
                                                          const std::string& ns,
                                                          bool reload,
                                                          bool forceReload) {
	return getChunkManager(txn, ns, reload, forceReload);
}

const ShardId& DBConfigCMongo::getShardId(OperationContext* txn, const std::string& ns) {
    uassert(40125, "ns can't be sharded", !isSharded(ns));

    uassert(40126, "no primary!", grid.shardRegistry()->getShard(txn, _primaryId));
    stdx::lock_guard<stdx::mutex> lk(_lock);
    return _primaryId;
}

void DBConfigCMongo::setPrimary(OperationContext* txn, const ShardId& newPrimaryId) {
    stdx::lock_guard<stdx::mutex> lk(_lock);
    _primaryId = newPrimaryId;
}

bool DBConfigCMongo::load(OperationContext* txn) {
	LOG(0) << "ERROR:DBConfigCMongo::load called";
	invariant(0);
	return false;
}

bool DBConfigCMongo::reload(OperationContext* txn) {
	LOG(0) << "ERROR:DBConfigCMongo::reload called";
	invariant(0);
	return false;
}

bool DBConfigCMongo::dropDatabase(OperationContext*, std::string& errmsg) {
	// TODO(deyukong): implement
	return false;
}

void DBConfigCMongo::getAllShardIds(std::set<ShardId>* shardIds) {
	// NOTE(deyukong): explict call to DBConfigCMongo::getAllShardIds only happens in
	// cmd.mapreduce
	// We can ban this command
	LOG(0) << "ERROR:DBConfigCMongo::getAllShardIds called";
	invariant(0);
}

void DBConfigCMongo::getAllShardedCollections(std::set<std::string>& namespaces) {
    stdx::lock_guard<stdx::mutex> lk(_lock);
	// NOTE(deyukong): consider non-shard tables in shard-collections
    for(CollectionInfoMap::iterator i = _collections.begin(); i != _collections.end(); ++i) {
		namespaces.insert(i->first);
	}
}

Status DBConfigCMongo::removeTable(const string& ns) {
    stdx::lock_guard<stdx::mutex> lk(_lock);
	CollectionInfoMap::iterator i = _collections.find(ns);
	if (i == _collections.end()) {
		return Status(ErrorCodes::NamespaceNotFound, "ERROR:" + ns + " not found");
	}
	LOG(0) << "removeTable:" << ns;
	_collections.erase(i);
	return Status::OK();
}

Status DBConfigCMongo::parseTableFromPB(const masterproto::TableRoutes& pb) {
    stdx::lock_guard<stdx::mutex> lk(_lock);
	CollectionInfoMap::iterator i = _collections.find(pb.ns());
	if (i != _collections.end()) {
		const std::unique_ptr<CollectionInfoCMongo>& coll = i->second;
		if (coll->routeMd5() == pb.md5()) {
			LOG(1) << pb.ns() << " md5 not change, ignore change";
			return Status::OK();
		}
	}

	std::unique_ptr<CollectionInfoCMongo> p(new CollectionInfoCMongo());
	Status s = p->parseFromPB(pb);
	if (!s.isOK()) {
		return s;
	}
	_collections[pb.ns()] = std::unique_ptr<CollectionInfoCMongo>(std::move(p));
	return Status::OK();
}
...
void ConfigServer::reloadSettings(OperationContext* txn) {
    ...
    if (chunkSizeResult.isOK()) {
        ...
        if (!ChunkDefault::setMaxChunkSizeSizeMB(csize)) {
...
void ConfigServer::replicaSetChangeConfigServerUpdateHook(const string& setName,
    ...
    try {
		...
		// NOTE(deyukong): lines below commented, no need to notify configSvr
		/*
