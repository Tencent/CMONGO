...
Status runApplyOpsCmd(OperationContext* txn,
    ...
    mergedChunk.setName(ChunkDefault::genID(firstChunk.getNS(), firstChunk.getMin()));
    ...
    for (size_t i = 1; i < chunksToMerge.size(); ++i) {
        ...
        chunkToMerge.setName(ChunkDefault::genID(chunkToMerge.getNS(), chunkToMerge.getMin()));
