...
GridDefault::GridDefault() : _allowLocalShard(true) {}
GridDefault::~GridDefault() = default;

void GridDefault::init(std::unique_ptr<ForwardingCatalogManager> catalogManager,
	...
	invariant(!_flowManager);

    _catalogManager = std::move(catalogManager);
    _catalogCache = stdx::make_unique<CatalogCacheCMongo>();
    _shardRegistry = std::move(shardRegistry);
    _cursorManager = std::move(cursorManager);
	_flowManager = flowManager;
}

StatusWith<std::shared_ptr<DBConfig>> GridDefault::implicitCreateDb(OperationContext* txn,
...
bool GridDefault::allowLocalHost() const {
    return _allowLocalShard;
}

void GridDefault::setAllowLocalHost(bool allow) {
 ...
 */
bool GridDefault::shouldBalance(const SettingsType& balancerSettings) const {
...
bool GridDefault::getConfigShouldBalance(OperationContext* txn) const {
...
void GridDefault::clearForUnitTests() {
...
ForwardingCatalogManager* GridDefault::forwardingCatalogManager() {
    return _catalogManager.get();
}

CatalogManager* GridDefault::catalogManager(OperationContext* txn) {
