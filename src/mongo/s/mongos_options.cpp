...
Status storeMongosOptions(const moe::Environment& params, const std::vector<std::string>& args) {
	...
	if (params.count("proxy.enableSharding")) {
		mongosGlobalParams.enableSharding = params["proxy.enableSharding"].as<int>();
	} else {
        return Status(ErrorCodes::BadValue, "proxy.enableSharding not found");
	}
	if (params.count("metaUrl")) {
		mongosGlobalParams.metaUrl = params["metaUrl"].as<std::string>();
	} else {
        return Status(ErrorCodes::BadValue, "metaUrl not found");
	}
	if (params.count("mongo.clusterName")) {
		mongosGlobalParams.clusterName = params["mongo.clusterName"].as<std::string>();
	} else {
        return Status(ErrorCodes::BadValue, "mongo.clusterName not found");
	}
	if (params.count("asio.workerNum")) {
		mongosGlobalParams.asioWorkerNum = params["asio.workerNum"].as<int>();
	} else {
		mongosGlobalParams.asioWorkerNum = 0;
	}
	if (params.count("asio.minConnPerHostPerWorker")) {
		mongosGlobalParams.minConnPerHostPerWorker = params["asio.minConnPerHostPerWorker"].as<int>();
	} else {
		mongosGlobalParams.minConnPerHostPerWorker = 1;
	}
	if (params.count("asio.maxConnPerHostPerWorker")) {
		mongosGlobalParams.maxConnPerHostPerWorker = params["asio.maxConnPerHostPerWorker"].as<int>();
	} else {
		mongosGlobalParams.maxConnPerHostPerWorker = std::numeric_limits<int>::max();
	}
	if (params.count("asio.maxQueueSizePerHostPerWorker")) {
	    mongosGlobalParams.asioMaxQueueSize = params["asio.maxQueueSizePerHostPerWorker"].as<int>();
	} else {
	    // 0 means disabled
	    mongosGlobalParams.asioMaxQueueSize = 0;
	}
	if (params.count("proxy.globalMaxQueueSizePerHost")) {
	    mongosGlobalParams.globalMaxQueueSize = params["proxy.globalMaxQueueSizePerHost"].as<int>();
	} else {
	    mongosGlobalParams.globalMaxQueueSize = 0;
	}
	if (params.count("proxy.shardMaxQueueSizePerHost")) {
        mongosGlobalParams.shardMaxQueueSize = params["proxy.shardMaxQueueSizePerHost"].as<int>();
    } else {
        mongosGlobalParams.shardMaxQueueSize = 0;
    }
    return Status::OK();
}
