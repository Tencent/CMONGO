...
CursorId ClusterFind::runQuery(OperationContext* opCtx,
	...
	if (dbname == "local" && query.nss().ns() != "local.oplog.rs") {
		uasserted(ErrorCodes::InvalidNamespace, str::stream() << "not allowed to visit" << query.nss().ns());
	}
	if (query.nss().ns() == "local.oplog.rs") {
		std::vector<ShardId> shards;
		shardRegistry->getAllShardIds(&shards);
		if (shards.size() != 1) {
			uasserted(ErrorCodes::InvalidNamespace, "can not read oplog on a sharded cluster");
		}
		return runQueryWithoutRetrying(
			opCtx, query, readPref, nullptr, uassertStatusOK(shardRegistry->getShard(opCtx, shards[0])), results);
	}
