...
StatusWith<CursorId> runQueryWithoutRetrying(OperationContext* txn,
    ...
    for (const auto& shard : shards) {
		...
		// NOTE(deyukong): no version should be attached in cmongo-proxy
		/*
        if (chunkManager) {
...
StatusWith<CursorId> ClusterFind::runQuery(OperationContext* txn,
	...
	const std::string& dbname = query.nss().db().toString();
	if (dbname == "admin") {
		return Status(ErrorCodes::InvalidNamespace, "not allowed to visit admin");
	}
	...
	uint64_t getDbTime = curTimeMicros64() - startTime;

	if (dbname == "local" && query.nss().ns() != "local.oplog.rs") {
		return Status(ErrorCodes::InvalidNamespace, str::stream() << "not allowed to visit" << query.nss().ns());
	}

	if (query.nss().ns() == "local.oplog.rs" && dbConfig.getValue()->isShardingEnabled()) {
		return Status(ErrorCodes::InvalidNamespace, "can not read oplog on a sharded cluster");
	}

    std::shared_ptr<ChunkManager> chunkManager;
    std::shared_ptr<Shard> primary;
    dbConfig.getValue()->getChunkManagerOrPrimary(txn, query.nss().ns(), chunkManager, primary);
	if (!chunkManager && !primary) {
		// NOTE(deyukong): the collection is not found
		return CursorId(0);
	}

	uint64_t getCMTime = curTimeMicros64() - startTime;
    ...
    for (size_t retries = 1; retries <= kMaxStaleConfigRetries; ++retries) {
		...
		if (delta >= 100000) {
			LOG(0)<<"getDbTime:"<<getDbTime<<",getCMTime:"<<getCMTime
				  <<",delta:"<<delta;
		}
