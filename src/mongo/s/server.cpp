...
void exitCleanly(ExitCode code) {
...

    {
		...
		auto flowManager = grid.getFlowManager();
...
public:
    ShardedMessageHandler() : _isAuthenticated(false) {}

    virtual ~ShardedMessageHandler() {}

    virtual void connected(AbstractMessagingPort* p) {
        Client::initThread("conn", getGlobalServiceContext(), p);
        grid.getFlowManager()->onConnected(p->connectionId(), p->remote());
    }
    ...
    virtual void process(Message& m, AbstractMessagingPort* p) {
        ...
        try {
            r.init(txn.get());
            r.process(txn.get());
			NamespaceString nss(StringData(r.getnsIfPresent()));
			grid.getFlowManager()->onSuccesses(nss);
            if (!_isAuthenticated && Client::getCurrent()) {
                long long connectionID = p->connectionId();
                auto authSession = AuthorizationSession::get(Client::getCurrent());
                auto users = authSession->getAuthenticatedUserNames();
                if (users.more()) {
                    grid.getFlowManager()->onAuthenticate(connectionID, users.next());
            ...
            LOG(ex.isUserAssertion() ? 1 : 0) << "Assertion failed"
                                              ...
                                              << " for " << nss << causedBy(ex);
			if (ex.isUserAssertion()) {
				grid.getFlowManager()->onClientError(nss);
			} else {
				grid.getFlowManager()->onServerError(nss);
			}
            if (r.expectResponse()) {
                m.header().setId(r.id());
    ...
    virtual void close() {
        ...
        grid.getFlowManager()->onDisconnected(Client::getCurrent()->port()->connectionId());
        Client::destroy();
    }
...
static Status initializeSharding(OperationContext* txn) {
    Status status = initializeGlobalShardingState(txn,
			mongosGlobalParams.configdbs,
			true,
			mongosGlobalParams.metaUrl,
			mongosGlobalParams.clusterName,
			mongosGlobalParams.enableSharding,
			mongosGlobalParams.minConnPerHostPerWorker,
			mongosGlobalParams.maxConnPerHostPerWorker,
			mongosGlobalParams.asioWorkerNum,
			mongosGlobalParams.asioMaxQueueSize);
    if (!status.isOK()) {
        return status;
    }
...
static ExitCode runMongosServer() {
...

    {
		...
		// NOTE(deyukong): cmongo no need to reloadSettings
        // ConfigServer::reloadSettings(txn.get());
    }

    globalConnPool.setNormalQueueSize(mongosGlobalParams.globalMaxQueueSize);
    shardConnectionPool.setNormalQueueSize(mongosGlobalParams.shardMaxQueueSize);

#if !defined(_WIN32)
	// TODO(deyukong): intergrate with gostyle-fork
	...
	// NOTE(deyukong): balancer is disabled in cmongo
    // balancer.go();
    ...
    MessageServer* server = createServer(opts, std::move(handler), grid.getFlowManager());
	...
	{
		LOG(0) << "ERROR:protobuf:" << filename << ":" << line << ":" << message;
	};
...
MONGO_INITIALIZER_GENERAL(ForkServer,
	...
	if (serverGlobalParams.doFork) {
		// NOTE(deyukong) mongo::forkServerOrDie forks, which will currupt golang's runtime
		// use golang-style fork
		cmongo::forkServerOrDie();
	}
    return Status::OK();
}
