...
Status initializeGlobalShardingState(OperationContext* opCtx,
    ...
    std::shared_ptr<FlowManager> flowManager = std::shared_ptr<FlowManager>(FlowManager::create(metaUrl, clusterName));
    ...
    // The flowManager  must be started after the shard registry has been started up
    Status s = flowManager->startup(opCtx);
    if (!s.isOK()) {
