...
std::unique_ptr<TaskExecutorPool> makeTaskExecutorPool(std::unique_ptr<NetworkInterface> fixedNet,
		...
		int minConnPerHostPerWorker, int maxConnPerHostPerWorker, int asioWorkerNum) {
    std::vector<std::unique_ptr<executor::TaskExecutor>> executors;
	size_t pool_size = asioWorkerNum;
	if (pool_size <= 0) {
		pool_size = TaskExecutorPool::getSuggestedPoolSize();
	}
	log() << "init asio pool, size:" << pool_size
		  << ",minConnPerHostPerWorker:" << minConnPerHostPerWorker
		  << ",maxConnPerHostPerWorker:" << maxConnPerHostPerWorker;
    for (size_t i = 0; i < pool_size; ++i) {
        auto net = executor::makeNetworkInterface(
            ...
            connPoolOptions,
			minConnPerHostPerWorker,
			maxConnPerHostPerWorker);
        auto netPtr = net.get();
...
Status initializeGlobalShardingState(OperationContext* txn,
									 ...
									 const std::string& etcdMeta,
									 const std::string& clusterName,
                                     bool enableSharding,
									 int minConnPerHostPerWorker,
									 int maxConnPerHostPerWorker,
									 int asioWorkerNum,
									 int asioMaxQueueSize) {
    ...
    connPoolOptions.overloadConfig.disableOverload();

    // The size of in-use pool is the same max size of pool by default
    connPoolOptions.overloadConfig.setNormalInUsePoolSize(
        (ShardingTaskExecutorPoolMaxConnecting != -1 &&
         ShardingTaskExecutorPoolMaxConnecting <= int(connPoolOptions.maxConnections))
        ? ShardingTaskExecutorPoolMaxConnecting
        : connPoolOptions.maxConnections);
    connPoolOptions.overloadConfig.setNormalQueueSize(asioMaxQueueSize);
	...
	// NOTE(deyukong): configCS is type of ConnectionString::SYNC
	// ShardRegistry won't directly call configSvr in itself in this mode
	// please see how CatalogManagerLegacy(CatalogManagerCMongo) and CatalogManagerReplicaSet
	// calls configSvr, it is very different
	// so it is ok to pass configCS to ShardRegistry here
    auto shardRegistry(
        stdx::make_unique<ShardRegistry>(stdx::make_unique<RemoteCommandTargeterFactoryImpl>(),
                                         makeTaskExecutorPool(std::move(network), connPoolOptions, minConnPerHostPerWorker, maxConnPerHostPerWorker, asioWorkerNum),
                                         networkPtr,
	...
	std::shared_ptr<FlowManager> flowManager = 
		std::make_shared<FlowManager>(etcdMeta, clusterName);
			
    shardRegistry->startup();
	Status s = flowManager->startup(txn);
	if (!s.isOK()) {
        log() << "Error initializing sharding state, init flowmanager failed:" << s;
		return s;
	}
