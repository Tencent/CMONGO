...
void ServiceEntryPointImpl::startSession(transport::SessionHandle session) {
    ...
    log() << "start connection from " << session->remote();

    // Checking if we successfully added a connection above. Separated from the lock so we don't log
    // while holding it.
    std::shared_ptr<FlowManager> fm = FlowManager::getInstance();
    std::string remoteAddress = remoteAddr->getAddr();
    if (remoteAddress == "(NONE)") {
        log() << "connection refused because remote address is unknown " << remoteAddress;
        return;
    }
    bool needTicket = fm && fm->needTicket(hostbyname(remoteAddress.c_str()));
    ...
    auto ssm = ServiceStateMachine::create(_svcCtx, session, transportMode);
    {
        ...
        if (connectionCount <= _maxNumConnections || !needTicket) {
            ssmIt = _sessions.emplace(_sessions.begin(), ssm);
            _currentConnections.store(connectionCount);
            _createdConnections.addAndFetch(1);
            if (!quiet && connectionCount > _maxNumConnections) {
                log() << "connection accepted when full " << remoteAddress;
            }
        } else {
            if (!quiet) {
...
void ServiceEntryPointImpl::resizeMaxConns(size_t max_conn) {
    stdx::unique_lock<decltype(_sessionsMutex)> lk(_sessionsMutex);
	_maxNumConnections = max_conn;
}

void ServiceEntryPointImpl::resizeMaxConnToCurrentUsed() {
    stdx::unique_lock<decltype(_sessionsMutex)> lk(_sessionsMutex);
	_maxNumConnections = _currentConnections.load();
}
...
ServiceEntryPoint::Stats ServiceEntryPointImpl::sessionStats() const {
    ...
    ret.numAvailableSessions = _maxNumConnections > sessionCount ? _maxNumConnections - sessionCount : 0;
    return ret;
}
