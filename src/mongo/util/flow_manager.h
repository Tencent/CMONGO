...
struct CMongoProxyStat {
	...
	BSONObj dump(OperationContext* opCtx) const;
};
...
public:
	static std::shared_ptr<FlowManager> create(const std::string& metaUrl, const std::string& clusterName);
	static std::shared_ptr<FlowManager> getInstance();
	...
	BSONObj dump() const;
    void dump(BSONObjBuilder* bbp,
              OperationContext* opCtx,
              DumpConnType dumpConnType = dumpConnNone,
              bool internal = false) const;
    void dump_inlock(BSONObjBuilder* bbp,
                     OperationContext* opCtx,
                     DumpConnType dumpConnType = dumpConnNone,
                     bool internal = false) const;
    void dumpConnection_inlock(BSONObjBuilder* bbp,
                               DumpConnType dumpConnType,
                               bool internal) const;
	// this function may throw exceptions
	virtual BSONObj getFilter(OperationContext* opCtx);
	virtual std::string getMonitorAddr(OperationContext* opCtx);
	virtual std::string getMasterAddr(OperationContext* opCtx);
	bool inShutdown() const;
	void shutdown();

    void onConnected(transport::SessionId connectionID,
                     const HostAndPort & client);
    void onDisconnected(transport::SessionId connectionID);
    void onAuthenticate(transport::SessionId connectionID,
                        const UserName & user);

private:
	FlowManager(const std::string& metaUrl, const std::string& cluster);
	...
	std::map<transport::SessionId, CMongoConnectionStat> _connectionStats;

	// Global instance
	static std::shared_ptr<FlowManager> _instance;
};
}
