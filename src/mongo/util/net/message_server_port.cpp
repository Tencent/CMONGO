...
public:
    MessagingPortWithHandler(const std::shared_ptr<Socket>& socket,
							 ...
							 bool hasTicket)
        : MessagingPort(socket), _handler(handler), _hasTicket(hasTicket) {
...
public:
     ...
     */
    PortMessageServer(const MessageServer::Options& opts, std::shared_ptr<MessageHandler> handler, std::shared_ptr<FlowManager> flowManager)
        : Listener("", opts.ipList, opts.port), _handler(std::move(handler)), _flowManager(std::move(flowManager)) {}
    ...
    virtual void accepted(std::shared_ptr<Socket> psocket, long long connectionId) {
        ...
        std::string remoteAddress = psocket->remoteAddr().getAddr();
        log() << "start connection from " << remoteAddress;
        if (remoteAddress == "(NONE)") {
            log() << "connection refused because remote address is unknown " << remoteAddress;
            return;
        }
        std::string remote = hostbyname(remoteAddress.c_str());
        bool needTicket = _flowManager ? _flowManager->needTicket(remote): true;
        std::unique_ptr<MessagingPortWithHandler> portWithHandler(
            new MessagingPortWithHandler(psocket, _handler, connectionId, needTicket));

        if (needTicket && !Listener::globalTicketHolder.tryAcquire()) {
...
private:
	...
	const std::shared_ptr<FlowManager> _flowManager;
    ...
    static void* handleIncomingMsg(void* arg) {
		unique_ptr<TicketHolderReleaser> pConnTicketReleaser;

        invariant(arg);
        unique_ptr<MessagingPortWithHandler> portWithHandler(
            static_cast<MessagingPortWithHandler*>(arg));
		if (portWithHandler->hasTicket()) {
			pConnTicketReleaser = std::move(stdx::make_unique<TicketHolderReleaser>(&Listener::globalTicketHolder));
		}
...
MessageServer* createServer(const MessageServer::Options& opts,
    ...
    return new PortMessageServer(opts, std::move(handler), std::move(flowManager));
}
