...
Status DBImpl::DelayWrite(uint64_t num_bytes) {
  ...
  bool delayed = false;
  {
    ...
    if (delay > 0) {
      ...
      // NOTE(zhenyipeng) hopefully we don't have to sleep more than 5 seconds,
      // As sleep too long is harmful for oplog syncing, that may lead to recovering
      if (delay > 5000000) {
        Log(InfoLogLevel::INFO_LEVEL, db_options_.info_log,
            "get delay %" PRIu64 " too large, set to 5,000,000\n", delay);
        delay = 5000000;
      }
      // NOTE(zhenyipeng) Merge from rocksdb5.14:
      // We will delay the write until we have slept for delay ms or
      // we don't need a delay anymore
      const int kDelayInterval = 1000;
      uint64_t stall_end = 0;
      if (sw.start_time() == 0) {
        stall_end = env_->NowMicros() + delay;
      } else {
        stall_end = sw.start_time() + delay;
      }
      while (write_controller_.NeedsDelay()) {
        ...
        env_->SleepForMicroseconds(kDelayInterval * 10);
      }
      mutex_.Lock();
    }
