...
Slice CompressBlock(const Slice& raw,
  ...
  switch (*type) {
	...
	case kAesSnappyCompression:
	  if (AesSnappy_Compress(compression_options, raw.data(), raw.size(), compressed_output)) {
		return *compressed_output;
	  }
	  // NOTE(deyukong): aes should always be applied, it's encryption more than compression
	  break;
