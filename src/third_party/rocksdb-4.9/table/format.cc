...
Status UncompressBlockContents(const char* data, size_t n,
  ...
  switch (data[n]) {
	...
	case kAesSnappyCompression: {
	  //static char aes_snappy_corrupt_msg[] =
      //  "AesSnappy not supported or corrupted AesSnappy compressed block contents";
	  std::string aes_output;
	  if (!AesDecrypt(data, n, &aes_output)) {
		return Status::Corruption("aes decrypt failed");
	  }
      size_t ulength = 0;
      if (!Snappy_GetUncompressedLength(aes_output.c_str(), aes_output.size(), &ulength)) {
        return Status::Corruption("get snappy length failed");
      }
	  ubuf.reset(new char[ulength]);
      if (!Snappy_Uncompress(aes_output.c_str(), aes_output.size(), ubuf.get())) {
        return Status::Corruption("uncompress snappy failed");
      }
      *contents = BlockContents(std::move(ubuf), ulength, true, kNoCompression);
      break;
	}
