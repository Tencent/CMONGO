...
inline bool AesSnappy_Supported() {
#ifdef SNAPPY
  return true;
#endif
  return false;
...
inline bool CompressionTypeSupported(CompressionType compression_type) {
  switch (compression_type) {
	...
	case kAesSnappyCompression:
      return AesSnappy_Supported();
...
inline void aes_free(EVP_CIPHER_CTX *ctx) {
	if(ctx) {
		EVP_CIPHER_CTX_cleanup(ctx);
	}
}

extern std::string global_enc_key;

inline int aes_init(unsigned char *salt, EVP_CIPHER_CTX *e_ctx, EVP_CIPHER_CTX *d_ctx) {
	int i, nrounds = 5;
	unsigned char key[32], iv[32];

	const unsigned char *key_data = reinterpret_cast<const unsigned char *>(global_enc_key.c_str());
	int key_data_len = global_enc_key.size();
...
inline bool AesSnappy_Compress(const CompressionOptions& opts, const char *input,
							size_t length, ::std::string* output) {
#ifndef SNAPPY
	return false;
#endif
	::std::string output1;
	bool ok = Snappy_Compress(opts, input, length, &output1);
    if (!ok) {
		return false;
	}
	EVP_CIPHER_CTX en;
	if (aes_init(NULL, &en, NULL) != 0) {
		return false;
	}
	int c_len = output1.size() +AES_BLOCK_SIZE, f_len = 0;
	std::unique_ptr<unsigned char[]> ubuf;
	ubuf.reset(new unsigned char[c_len]);
	EVP_EncryptInit_ex(&en, NULL, NULL, NULL, NULL);
	EVP_EncryptUpdate(&en, ubuf.get(), &c_len, reinterpret_cast<const unsigned char *>(output1.c_str()), output1.size());
	EVP_EncryptFinal_ex(&en, ubuf.get()+c_len, &f_len);
	*output = ::std::string(reinterpret_cast<const char *>(ubuf.get()), c_len + f_len);
	aes_free(&en);
	return true;
}

inline bool AesDecrypt(const char *input, size_t length, ::std::string* output) {
	EVP_CIPHER_CTX de;
	if (aes_init(NULL, NULL, &de) != 0) {
		return false;
	}
	int p_len = length, f_len = 0;
	std::unique_ptr<unsigned char[]> ubuf;
	ubuf.reset(new unsigned char[p_len]);
	EVP_DecryptInit_ex(&de, NULL, NULL, NULL, NULL);
	EVP_DecryptUpdate(&de, ubuf.get(), &p_len, reinterpret_cast<const unsigned char *>(input), length);
	EVP_DecryptFinal_ex(&de, ubuf.get()+p_len, &f_len);
	*output = ::std::string(reinterpret_cast<const char *>(ubuf.get()), p_len + f_len);
	aes_free(&de);
	return true;
}
